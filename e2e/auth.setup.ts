...
setup("authenticate as admin", async ({ page }) => {
  ...
  // Click the Login button in the sidebar to open the AuthDialog
  await page
    .locator('[data-sidebar="footer"] button', { hasText: "Login" })
    .click();

  // The AuthDialog has Login and Register tabs with identical placeholders.
  ...
  await expect(
    page.locator('[data-sidebar="footer"] button', { hasText: "Logout" }),
  ).toBeVisible({
    timeout: 15_000,
  });
