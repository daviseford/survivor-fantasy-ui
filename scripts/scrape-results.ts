...
export async function scrapeResults(
  ...
  if (mergeEpisode) {
    // Players who survive past the merge episode get make_merge events
    // (this requires knowing who is still in the game, which we can derive from eliminations)
    ...
    // We don't know all player names from the episode guide alone.
    // mark this as a warning — the orchestrator can fill this in from player data
    if (eliminatedBefore.size > 0) {
      warnings.push(
