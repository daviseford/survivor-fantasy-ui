import {
  CastawayLookup,
  Challenge,
  Elimination,
  Episode,
  GameEvent,
  Player,
} from "../../types";

// eslint-disable-next-line @typescript-eslint/no-unused-vars -- used only in typeof for type derivation
const CastawayIds = [
  "US0282",
  "US0283",
  "US0284",
  "US0285",
  "US0286",
  "US0287",
  "US0288",
  "US0289",
  "US0290",
  "US0291",
  "US0292",
  "US0293",
  "US0294",
  "US0295",
  "US0296",
  "US0297",
  "US0298",
  "US0299",
  "US0300",
  "US0301",
] as const;

type CastawayIdType = (typeof CastawayIds)[number];

type SeasonNumber = 19;

const buildPlayer = <T extends CastawayIdType>(
  p: { castaway_id: T; full_name: string; img: string } & Partial<
    Omit<
      Player<T, SeasonNumber>,
      "season_id" | "season_num" | "castaway_id" | "full_name" | "img"
    >
  >,
): Player<T, SeasonNumber> => ({
  ...p,
  season_num: 19,
  season_id: "season_19",
});

export const SEASON_19_CASTAWAY_LOOKUP: CastawayLookup = {
  US0282: { full_name: "Marisa Calihan", castaway: "Marisa" },
  US0283: { full_name: "Mike Borassi", castaway: "Mike" },
  US0284: { full_name: "Betsy Bolan", castaway: "Betsy" },
  US0285: { full_name: "Ben Browning", castaway: "Ben" },
  US0286: { full_name: "Yasmin Giles", castaway: "Yasmin" },
  US0287: { full_name: "Ashley Trainer", castaway: "Ashley" },
  US0288: { full_name: "Russell Swan", castaway: "Russell S." },
  US0289: { full_name: "Elizabeth Kim", castaway: "Liz" },
  US0290: { full_name: "Erik Cardona", castaway: "Erik" },
  US0291: { full_name: "Kelly Sharbaugh", castaway: "Kelly" },
  US0292: { full_name: "Laura Morett", castaway: "Laura" },
  US0293: { full_name: "John Fincher", castaway: "John" },
  US0294: { full_name: "Dave Ball", castaway: "Dave" },
  US0295: { full_name: "Monica Padilla", castaway: "Monica" },
  US0296: { full_name: "Shannon Waters", castaway: "Shambo" },
  US0297: { full_name: "Jaison Robinson", castaway: "Jaison" },
  US0298: { full_name: "Brett Clouser", castaway: "Brett" },
  US0299: { full_name: "Mick Trimming", castaway: "Mick" },
  US0300: { full_name: "Russell Hantz", castaway: "Russell H." },
  US0301: { full_name: "Natalie White", castaway: "Natalie" },
};

export const SEASON_19_PLAYERS = [
  buildPlayer({
    castaway_id: "US0282",
    full_name: "Marisa Calihan",
    img: "/images/season_19/Marisa-Calihan.jpg",
    description: "Age: 26 | Hometown: Cincinnati, Ohio | Occupation: Student",
    age: 26,
    profession: "Student",
    hometown: "Cincinnati, Ohio",
  }),
  buildPlayer({
    castaway_id: "US0283",
    full_name: "Mike Borassi",
    img: "/images/season_19/Mike-Borassi.jpg",
    description:
      "Age: 62 | Hometown: Marina del Rey, California | Occupation: Personal Chef",
    age: 62,
    profession: "Personal Chef",
    hometown: "Marina del Rey, California",
  }),
  buildPlayer({
    castaway_id: "US0284",
    full_name: "Betsy Bolan",
    img: "/images/season_19/Betsy-Bolan.jpg",
    description:
      "Age: 48 | Hometown: Campton, New Hampshire | Occupation: Police Officer",
    age: 48,
    profession: "Police Officer",
    hometown: "Campton, New Hampshire",
  }),
  buildPlayer({
    castaway_id: "US0285",
    full_name: "Ben Browning",
    img: "/images/season_19/Ben-Browning.jpg",
    description:
      "Age: 28 | Hometown: Los Angeles, California | Occupation: Bar Manager",
    age: 28,
    profession: "Bar Manager",
    hometown: "Los Angeles, California",
    nickname: "Ben",
  }),
  buildPlayer({
    castaway_id: "US0286",
    full_name: "Yasmin Giles",
    img: "/images/season_19/Yasmin-Giles.jpg",
    description:
      "Age: 33 | Hometown: Los Angeles, California | Occupation: Hairstylist",
    age: 33,
    profession: "Hairstylist",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0287",
    full_name: "Ashley Trainer",
    img: "/images/season_19/Ashley-Trainer.jpg",
    description:
      "Age: 22 | Hometown: Maple Grove, Minnesota | Occupation: Spa Sales",
    age: 22,
    profession: "Spa Sales",
    hometown: "Maple Grove, Minnesota",
  }),
  buildPlayer({
    castaway_id: "US0288",
    full_name: "Russell Swan",
    img: "/images/season_19/Russell-Swan.jpg",
    description:
      "Age: 42 | Hometown: Glenside, Pennsylvania | Occupation: Attorney",
    age: 42,
    profession: "Attorney",
    hometown: "Glenside, Pennsylvania",
    nickname: "Russell S.",
  }),
  buildPlayer({
    castaway_id: "US0289",
    full_name: "Elizabeth Kim",
    img: "/images/season_19/Elizabeth-Kim.jpg",
    description:
      "Age: 33 | Hometown: New York City, New York | Occupation: Urban Planner",
    age: 33,
    profession: "Urban Planner",
    hometown: "New York City, New York",
    nickname: "Liz",
  }),
  buildPlayer({
    castaway_id: "US0290",
    full_name: "Erik Cardona",
    img: "/images/season_19/Erik-Cardona.jpg",
    description:
      "Age: 28 | Hometown: Los Angeles, California | Occupation: Bartender",
    age: 28,
    profession: "Bartender",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0291",
    full_name: "Kelly Sharbaugh",
    img: "/images/season_19/Kelly-Sharbaugh.jpg",
    description:
      "Age: 25 | Hometown: San Diego, California | Occupation: Hairstylist",
    age: 25,
    profession: "Hairstylist",
    hometown: "San Diego, California",
  }),
  buildPlayer({
    castaway_id: "US0292",
    full_name: "Laura Morett",
    img: "/images/season_19/Laura-Morett.jpg",
    description:
      "Age: 39 | Hometown: Salem, Oregon | Occupation: Office Manager",
    age: 39,
    profession: "Office Manager",
    hometown: "Salem, Oregon",
  }),
  buildPlayer({
    castaway_id: "US0293",
    full_name: "John Fincher",
    img: "/images/season_19/John-Fincher.jpg",
    description:
      "Age: 25 | Hometown: Santa Monica, California | Occupation: Rocket Scientist",
    age: 25,
    profession: "Rocket Scientist",
    hometown: "Santa Monica, California",
  }),
  buildPlayer({
    castaway_id: "US0294",
    full_name: "Dave Ball",
    img: "/images/season_19/Dave-Ball.jpg",
    description:
      "Age: 38 | Hometown: Los Angeles, California | Occupation: Fitness Instructor",
    age: 38,
    profession: "Fitness Instructor",
    hometown: "Los Angeles, California",
    nickname: "Dave",
  }),
  buildPlayer({
    castaway_id: "US0295",
    full_name: "Monica Padilla",
    img: "/images/season_19/Monica-Padilla.jpg",
    description:
      "Age: 25 | Hometown: San Diego, California | Occupation: Law Student",
    age: 25,
    profession: "Law Student",
    hometown: "San Diego, California",
  }),
  buildPlayer({
    castaway_id: "US0296",
    full_name: "Shannon Waters",
    img: "/images/season_19/Shannon-Waters.jpg",
    description: "Age: 45 | Hometown: Renton, Washington | Occupation: Sales",
    age: 45,
    profession: "Sales",
    hometown: "Renton, Washington",
    nickname: "Shambo",
  }),
  buildPlayer({
    castaway_id: "US0297",
    full_name: "Jaison Robinson",
    img: "/images/season_19/Jaison-Robinson.jpg",
    description:
      "Age: 28 | Hometown: Chicago, Illinois | Occupation: Law Student",
    age: 28,
    profession: "Law Student",
    hometown: "Chicago, Illinois",
  }),
  buildPlayer({
    castaway_id: "US0298",
    full_name: "Brett Clouser",
    img: "/images/season_19/Brett-Clouser.jpg",
    description:
      "Age: 23 | Hometown: Los Angeles, California | Occupation: T-Shirt Designer",
    age: 23,
    profession: "T-Shirt Designer",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0299",
    full_name: "Mick Trimming",
    img: "/images/season_19/Mick-Trimming.jpg",
    description:
      "Age: 33 | Hometown: Los Angeles, California | Occupation: Doctor",
    age: 33,
    profession: "Doctor",
    hometown: "Los Angeles, California",
    nickname: "Mick",
  }),
  buildPlayer({
    castaway_id: "US0300",
    full_name: "Russell Hantz",
    img: "/images/season_19/Russell-Hantz.jpg",
    description:
      "Age: 36 | Hometown: Dayton, Texas | Occupation: Oil Company Owner",
    age: 36,
    profession: "Oil Company Owner",
    hometown: "Dayton, Texas",
    nickname: "Russell H.",
  }),
  buildPlayer({
    castaway_id: "US0301",
    full_name: "Natalie White",
    img: "/images/season_19/Natalie-White.jpg",
    description:
      "Age: 26 | Hometown: Van Buren, Arkansas | Occupation: Pharmaceutical Sales",
    age: 26,
    profession: "Pharmaceutical Sales",
    hometown: "Van Buren, Arkansas",
  }),
] satisfies Player<CastawayIdType, SeasonNumber>[];

export const SEASON_19_EPISODES = [
  {
    id: "episode_1",
    season_id: "season_19",
    season_num: 19,
    order: 1,
    name: "The Puppet Master",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_2",
    season_id: "season_19",
    season_num: 19,
    order: 2,
    name: "Taking Candy From a Baby",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_3",
    season_id: "season_19",
    season_num: 19,
    order: 3,
    name: "It's Called a Russell Seed",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_4",
    season_id: "season_19",
    season_num: 19,
    order: 4,
    name: "Hungry for a Win",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_5",
    season_id: "season_19",
    season_num: 19,
    order: 5,
    name: "Walking on Thin Ice",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_6",
    season_id: "season_19",
    season_num: 19,
    order: 6,
    name: "This Is the Man Test",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_7",
    season_id: "season_19",
    season_num: 19,
    order: 7,
    name: "Houdini Magic",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_8",
    season_id: "season_19",
    season_num: 19,
    order: 8,
    name: "All Hell Breaks Loose",
    post_merge: true,
    finale: false,
    merge_occurs: true,
  },
  {
    id: "episode_9",
    season_id: "season_19",
    season_num: 19,
    order: 9,
    name: "Tastes Like Chicken",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_10",
    season_id: "season_19",
    season_num: 19,
    order: 10,
    name: "The Day of Reckoning",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_11",
    season_id: "season_19",
    season_num: 19,
    order: 11,
    name: "The First 27 Days",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_12",
    season_id: "season_19",
    season_num: 19,
    order: 12,
    name: "Off With Their Heads!",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_13",
    season_id: "season_19",
    season_num: 19,
    order: 13,
    name: "Damage Control",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_14",
    season_id: "season_19",
    season_num: 19,
    order: 14,
    name: "Two Brains Are Better Than One",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_15",
    season_id: "season_19",
    season_num: 19,
    order: 15,
    name: "This Game Ain't Over",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_16",
    season_id: "season_19",
    season_num: 19,
    order: 16,
    name: "Reunion",
    post_merge: false,
    finale: true,
    merge_occurs: false,
  },
] satisfies Episode<SeasonNumber>[];

export const SEASON_19_CHALLENGES = {
  challenge_0: {
    id: "challenge_0",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 0,
    winning_castaways: [
      "US0282",
      "US0283",
      "US0284",
      "US0285",
      "US0287",
      "US0289",
      "US0297",
      "US0299",
      "US0300",
      "US0301",
    ],
  },
  challenge_1: {
    id: "challenge_1",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 1,
    winning_castaways: [
      "US0286",
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_2: {
    id: "challenge_2",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 2,
    winning_castaways: [
      "US0286",
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_3: {
    id: "challenge_3",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 3,
    winning_castaways: [
      "US0286",
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_4: {
    id: "challenge_4",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 4,
    winning_castaways: [
      "US0286",
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_5: {
    id: "challenge_5",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 5,
    winning_castaways: [
      "US0286",
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_6: {
    id: "challenge_6",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_reward",
    order: 6,
    winning_castaways: [
      "US0286",
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_7: {
    id: "challenge_7",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_immunity",
    order: 7,
    winning_castaways: [
      "US0287",
      "US0289",
      "US0297",
      "US0299",
      "US0300",
      "US0301",
    ],
  },
  challenge_8: {
    id: "challenge_8",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_reward",
    order: 8,
    winning_castaways: [
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_9: {
    id: "challenge_9",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_immunity",
    order: 9,
    winning_castaways: [
      "US0288",
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_10: {
    id: "challenge_10",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "team_reward",
    order: 10,
    winning_castaways: [
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_11: {
    id: "challenge_11",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "team_immunity",
    order: 11,
    winning_castaways: [
      "US0290",
      "US0291",
      "US0292",
      "US0293",
      "US0294",
      "US0295",
      "US0296",
      "US0298",
    ],
  },
  challenge_12: {
    id: "challenge_12",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "immunity",
    order: 12,
    winning_castaways: ["US0292", "US0293"],
  },
  challenge_13: {
    id: "challenge_13",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "team_reward",
    order: 13,
    winning_castaways: ["US0291", "US0293", "US0294", "US0295", "US0296"],
  },
  challenge_14: {
    id: "challenge_14",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "immunity",
    order: 14,
    winning_castaways: ["US0292"],
  },
  challenge_15: {
    id: "challenge_15",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "team_reward",
    order: 15,
    winning_castaways: ["US0292", "US0294", "US0298", "US0300", "US0301"],
  },
  challenge_16: {
    id: "challenge_16",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "immunity",
    order: 16,
    winning_castaways: ["US0299"],
  },
  challenge_17: {
    id: "challenge_17",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "immunity",
    order: 17,
    winning_castaways: ["US0297"],
  },
  challenge_18: {
    id: "challenge_18",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "immunity",
    order: 18,
    winning_castaways: ["US0297"],
  },
  challenge_19: {
    id: "challenge_19",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "immunity",
    order: 19,
    winning_castaways: ["US0298"],
  },
  challenge_20: {
    id: "challenge_20",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "team_reward",
    order: 20,
    winning_castaways: ["US0296", "US0297", "US0300"],
  },
  challenge_21: {
    id: "challenge_21",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "immunity",
    order: 21,
    winning_castaways: ["US0298"],
  },
  challenge_22: {
    id: "challenge_22",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    variant: "immunity",
    order: 22,
    winning_castaways: ["US0298"],
  },
  challenge_23: {
    id: "challenge_23",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    variant: "immunity",
    order: 23,
    winning_castaways: ["US0300"],
  },
} satisfies Record<Challenge["id"], Challenge<CastawayIdType, SeasonNumber>>;

export const SEASON_19_ELIMINATIONS = {
  elimination_1: {
    id: "elimination_1",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_1",
    episode_num: 1,
    order: 1,
    castaway_id: "US0282",
    variant: "tribal",
  },
  elimination_2: {
    id: "elimination_2",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_2",
    episode_num: 2,
    order: 2,
    castaway_id: "US0283",
    variant: "medical",
  },
  elimination_3: {
    id: "elimination_3",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_2",
    episode_num: 2,
    order: 3,
    castaway_id: "US0284",
    variant: "tribal",
  },
  elimination_4: {
    id: "elimination_4",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_3",
    episode_num: 3,
    order: 4,
    castaway_id: "US0285",
    variant: "tribal",
  },
  elimination_5: {
    id: "elimination_5",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_4",
    episode_num: 4,
    order: 5,
    castaway_id: "US0286",
    variant: "tribal",
  },
  elimination_6: {
    id: "elimination_6",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_5",
    episode_num: 5,
    order: 6,
    castaway_id: "US0287",
    variant: "tribal",
  },
  elimination_7: {
    id: "elimination_7",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_6",
    episode_num: 6,
    order: 7,
    castaway_id: "US0288",
    variant: "medical",
  },
  elimination_8: {
    id: "elimination_8",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_7",
    episode_num: 7,
    order: 8,
    castaway_id: "US0289",
    variant: "tribal",
  },
  elimination_9: {
    id: "elimination_9",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    order: 9,
    castaway_id: "US0290",
    variant: "tribal",
  },
  elimination_10: {
    id: "elimination_10",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_9",
    episode_num: 9,
    order: 10,
    castaway_id: "US0291",
    variant: "tribal",
  },
  elimination_11: {
    id: "elimination_11",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_10",
    episode_num: 10,
    order: 11,
    castaway_id: "US0292",
    variant: "tribal",
  },
  elimination_12: {
    id: "elimination_12",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_12",
    episode_num: 12,
    order: 12,
    castaway_id: "US0293",
    variant: "tribal",
  },
  elimination_13: {
    id: "elimination_13",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_13",
    episode_num: 13,
    order: 13,
    castaway_id: "US0294",
    variant: "tribal",
  },
  elimination_14: {
    id: "elimination_14",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_13",
    episode_num: 13,
    order: 14,
    castaway_id: "US0295",
    variant: "tribal",
  },
  elimination_15: {
    id: "elimination_15",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_14",
    episode_num: 14,
    order: 15,
    castaway_id: "US0296",
    variant: "tribal",
  },
  elimination_16: {
    id: "elimination_16",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    order: 16,
    castaway_id: "US0297",
    variant: "tribal",
  },
  elimination_17: {
    id: "elimination_17",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    order: 17,
    castaway_id: "US0298",
    variant: "tribal",
  },
  elimination_18: {
    id: "elimination_18",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    order: 18,
    castaway_id: "US0299",
    variant: "final_tribal_council",
  },
  elimination_19: {
    id: "elimination_19",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    order: 19,
    castaway_id: "US0300",
    variant: "final_tribal_council",
  },
} satisfies Record<
  Elimination["id"],
  Elimination<CastawayIdType, SeasonNumber>
>;

export const SEASON_19_EVENTS = {
  event_1: {
    id: "event_1",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0300",
    action: "find_idol",
    multiplier: null,
  },
  event_2: {
    id: "event_2",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0300",
    action: "use_idol",
    multiplier: null,
  },
  event_3: {
    id: "event_3",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_4",
    episode_num: 4,
    castaway_id: "US0290",
    action: "find_idol",
    multiplier: null,
  },
  event_4: {
    id: "event_4",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0290",
    action: "voted_out_with_idol",
    multiplier: null,
  },
  event_5: {
    id: "event_5",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_9",
    episode_num: 9,
    castaway_id: "US0300",
    action: "find_idol",
    multiplier: null,
  },
  event_6: {
    id: "event_6",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_9",
    episode_num: 9,
    castaway_id: "US0300",
    action: "use_idol",
    multiplier: null,
  },
  event_7: {
    id: "event_7",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0300",
    action: "find_idol",
    multiplier: null,
  },
  event_8: {
    id: "event_8",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_9",
    episode_num: 9,
    castaway_id: "US0300",
    action: "votes_negated_by_idol",
    multiplier: 7,
  },
  event_9: {
    id: "event_9",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0290",
    action: "make_merge",
    multiplier: null,
  },
  event_10: {
    id: "event_10",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0291",
    action: "make_merge",
    multiplier: null,
  },
  event_11: {
    id: "event_11",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0292",
    action: "make_merge",
    multiplier: null,
  },
  event_12: {
    id: "event_12",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0293",
    action: "make_merge",
    multiplier: null,
  },
  event_13: {
    id: "event_13",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0294",
    action: "make_merge",
    multiplier: null,
  },
  event_14: {
    id: "event_14",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0295",
    action: "make_merge",
    multiplier: null,
  },
  event_15: {
    id: "event_15",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0296",
    action: "make_merge",
    multiplier: null,
  },
  event_16: {
    id: "event_16",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0297",
    action: "make_merge",
    multiplier: null,
  },
  event_17: {
    id: "event_17",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0298",
    action: "make_merge",
    multiplier: null,
  },
  event_18: {
    id: "event_18",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0299",
    action: "make_merge",
    multiplier: null,
  },
  event_19: {
    id: "event_19",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0300",
    action: "make_merge",
    multiplier: null,
  },
  event_20: {
    id: "event_20",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0301",
    action: "make_merge",
    multiplier: null,
  },
  event_21: {
    id: "event_21",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0301",
    action: "win_survivor",
    multiplier: null,
  },
  event_22: {
    id: "event_22",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0299",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_23: {
    id: "event_23",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0300",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_24: {
    id: "event_24",
    season_id: "season_19",
    season_num: 19,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0301",
    action: "make_final_tribal_council",
    multiplier: null,
  },
} satisfies Record<GameEvent["id"], GameEvent<CastawayIdType, SeasonNumber>>;
