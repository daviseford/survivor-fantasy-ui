import {
  CastawayLookup,
  Challenge,
  Elimination,
  Episode,
  GameEvent,
  Player,
} from "../../types";

// eslint-disable-next-line @typescript-eslint/no-unused-vars -- used only in typeof for type derivation
const CastawayIds = [
  "US0354",
  "US0355",
  "US0356",
  "US0357",
  "US0358",
  "US0359",
  "US0360",
  "US0361",
  "US0362",
  "US0363",
  "US0364",
  "US0365",
  "US0366",
  "US0367",
  "US0368",
  "US0369",
  "US0370",
  "US0371",
] as const;

type CastawayIdType = (typeof CastawayIds)[number];

type SeasonNumber = 24;

const buildPlayer = <T extends CastawayIdType>(
  p: { castaway_id: T; full_name: string; img: string } & Partial<
    Omit<
      Player<T, SeasonNumber>,
      "season_id" | "season_num" | "castaway_id" | "full_name" | "img"
    >
  >,
): Player<T, SeasonNumber> => ({
  ...p,
  season_num: 24,
  season_id: "season_24",
});

export const SEASON_24_CASTAWAY_LOOKUP: CastawayLookup = {
  US0354: { full_name: "Kourtney Moon", castaway: "Kourtney" },
  US0355: { full_name: "Nina Acosta", castaway: "Nina" },
  US0356: { full_name: "Matt Quinlan", castaway: "Matt" },
  US0357: { full_name: "Bill Posley", castaway: "Bill" },
  US0358: { full_name: "Monica Culpepper", castaway: "Monica" },
  US0359: { full_name: "Colton Cumbie", castaway: "Colton" },
  US0360: { full_name: "Jonas Otsuji", castaway: "Jonas" },
  US0361: { full_name: "Michael Jefferson", castaway: "Michael" },
  US0362: { full_name: "Jay Byars", castaway: "Jay" },
  US0363: { full_name: "Leif Manson", castaway: "Leif" },
  US0364: { full_name: "Troy Robertson", castaway: "Troyzan" },
  US0365: { full_name: "Kat Edorsson", castaway: "Kat" },
  US0366: { full_name: "Greg Smith", castaway: "Tarzan" },
  US0367: { full_name: "Alicia Rosa", castaway: "Alicia" },
  US0368: { full_name: "Christina Cha", castaway: "Christina" },
  US0369: { full_name: "Chelsea Meissner", castaway: "Chelsea" },
  US0370: { full_name: "Sabrina Thompson", castaway: "Sabrina" },
  US0371: { full_name: "Kim Spradlin", castaway: "Kim" },
};

export const SEASON_24_PLAYERS = [
  buildPlayer({
    castaway_id: "US0354",
    full_name: "Kourtney Moon",
    img: "/images/season_24/Kourtney-Moon.jpg",
    description:
      "Age: 29 | Hometown: Austin, Texas | Occupation: Motorcycle Repair",
    age: 29,
    profession: "Motorcycle Repair",
    hometown: "Austin, Texas",
  }),
  buildPlayer({
    castaway_id: "US0355",
    full_name: "Nina Acosta",
    img: "/images/season_24/Nina-Acosta.jpg",
    description:
      "Age: 51 | Hometown: Clovis, California | Occupation: Retired LAPD Officer",
    age: 51,
    profession: "Retired LAPD Officer",
    hometown: "Clovis, California",
  }),
  buildPlayer({
    castaway_id: "US0356",
    full_name: "Matt Quinlan",
    img: "/images/season_24/Matt-Quinlan.jpg",
    description:
      "Age: 33 | Hometown: San Francisco, California | Occupation: Attorney",
    age: 33,
    profession: "Attorney",
    hometown: "San Francisco, California",
  }),
  buildPlayer({
    castaway_id: "US0357",
    full_name: "Bill Posley",
    img: "/images/season_24/Bill-Posley.jpg",
    description:
      "Age: 28 | Hometown: Venice, California | Occupation: Stand-Up Comedian",
    age: 28,
    profession: "Stand-Up Comedian",
    hometown: "Venice, California",
  }),
  buildPlayer({
    castaway_id: "US0358",
    full_name: "Monica Culpepper",
    img: "/images/season_24/Monica-Culpepper.jpg",
    description:
      "Age: 41 | Hometown: Tampa, Florida | Occupation: Ex-NFL Player's Wife",
    age: 41,
    profession: "Ex-NFL Player's Wife",
    hometown: "Tampa, Florida",
  }),
  buildPlayer({
    castaway_id: "US0359",
    full_name: "Colton Cumbie",
    img: "/images/season_24/Colton-Cumbie.jpg",
    description:
      "Age: 21 | Hometown: Monroeville, Alabama | Occupation: Student",
    age: 21,
    profession: "Student",
    hometown: "Monroeville, Alabama",
  }),
  buildPlayer({
    castaway_id: "US0360",
    full_name: "Jonas Otsuji",
    img: "/images/season_24/Jonas-Otsuji.jpg",
    description: "Age: 37 | Hometown: Lehi, Utah | Occupation: Sushi Chef",
    age: 37,
    profession: "Sushi Chef",
    hometown: "Lehi, Utah",
  }),
  buildPlayer({
    castaway_id: "US0361",
    full_name: "Michael Jefferson",
    img: "/images/season_24/Michael-Jefferson.jpg",
    description: "Age: 30 | Hometown: Seattle, Washington | Occupation: Banker",
    age: 30,
    profession: "Banker",
    hometown: "Seattle, Washington",
  }),
  buildPlayer({
    castaway_id: "US0362",
    full_name: "Jay Byars",
    img: "/images/season_24/Jay-Byars.jpg",
    description:
      "Age: 25 | Hometown: Gaffney, South Carolina | Occupation: Model",
    age: 25,
    profession: "Model",
    hometown: "Gaffney, South Carolina",
  }),
  buildPlayer({
    castaway_id: "US0363",
    full_name: "Leif Manson",
    img: "/images/season_24/Leif-Manson.jpg",
    description:
      "Age: 27 | Hometown: San Diego, California | Occupation: Phlebotomist",
    age: 27,
    profession: "Phlebotomist",
    hometown: "San Diego, California",
  }),
  buildPlayer({
    castaway_id: "US0364",
    full_name: "Troy Robertson",
    img: "/images/season_24/Troy-Robertson.jpg",
    description:
      "Age: 50 | Hometown: Miami, Florida | Occupation: Swimsuit Photographer",
    age: 50,
    profession: "Swimsuit Photographer",
    hometown: "Miami, Florida",
    nickname: "Troyzan",
  }),
  buildPlayer({
    castaway_id: "US0365",
    full_name: "Kat Edorsson",
    img: "/images/season_24/Kat-Edorsson.jpg",
    description:
      "Age: 22 | Hometown: Orlando, Florida | Occupation: Timeshare Representative",
    age: 22,
    profession: "Timeshare Representative",
    hometown: "Orlando, Florida",
  }),
  buildPlayer({
    castaway_id: "US0366",
    full_name: "Greg Smith",
    img: "/images/season_24/Greg-Smith.jpg",
    description:
      "Age: 64 | Hometown: Houston, Texas | Occupation: Plastic Surgeon",
    age: 64,
    profession: "Plastic Surgeon",
    hometown: "Houston, Texas",
    nickname: "Tarzan",
  }),
  buildPlayer({
    castaway_id: "US0367",
    full_name: "Alicia Rosa",
    img: "/images/season_24/Alicia-Rosa.jpg",
    description:
      "Age: 25 | Hometown: Chicago, Illinois | Occupation: Special Education Teacher",
    age: 25,
    profession: "Special Education Teacher",
    hometown: "Chicago, Illinois",
  }),
  buildPlayer({
    castaway_id: "US0368",
    full_name: "Christina Cha",
    img: "/images/season_24/Christina-Cha.jpg",
    description:
      "Age: 29 | Hometown: Hollywood, California | Occupation: Career Consultant",
    age: 29,
    profession: "Career Consultant",
    hometown: "Hollywood, California",
  }),
  buildPlayer({
    castaway_id: "US0369",
    full_name: "Chelsea Meissner",
    img: "/images/season_24/Chelsea-Meissner.jpg",
    description:
      "Age: 26 | Hometown: Charleston, South Carolina | Occupation: Medical Sales",
    age: 26,
    profession: "Medical Sales",
    hometown: "Charleston, South Carolina",
  }),
  buildPlayer({
    castaway_id: "US0370",
    full_name: "Sabrina Thompson",
    img: "/images/season_24/Sabrina-Thompson.jpg",
    description:
      "Age: 33 | Hometown: Brooklyn, New York | Occupation: High School Teacher",
    age: 33,
    profession: "High School Teacher",
    hometown: "Brooklyn, New York",
  }),
  buildPlayer({
    castaway_id: "US0371",
    full_name: "Kim Spradlin",
    img: "/images/season_24/Kim-Spradlin.jpg",
    description:
      "Age: 29 | Hometown: San Antonio, Texas | Occupation: Bridal Shop Owner",
    age: 29,
    profession: "Bridal Shop Owner",
    hometown: "San Antonio, Texas",
  }),
] satisfies Player<CastawayIdType, SeasonNumber>[];

export const SEASON_24_EPISODES = [
  {
    id: "episode_1",
    season_id: "season_24",
    season_num: 24,
    order: 1,
    name: "Two Tribes, One Camp, No Rules",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_2",
    season_id: "season_24",
    season_num: 24,
    order: 2,
    name: "Total Dysfunction",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_3",
    season_id: "season_24",
    season_num: 24,
    order: 3,
    name: "One World is Out the Window",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_4",
    season_id: "season_24",
    season_num: 24,
    order: 4,
    name: "Bum-Puzzled",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_5",
    season_id: "season_24",
    season_num: 24,
    order: 5,
    name: "A Bunch of Idiots",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_6",
    season_id: "season_24",
    season_num: 24,
    order: 6,
    name: "Thanks for the Souvenir",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_7",
    season_id: "season_24",
    season_num: 24,
    order: 7,
    name: "The Beauty in a Merge",
    post_merge: true,
    finale: false,
    merge_occurs: true,
  },
  {
    id: "episode_8",
    season_id: "season_24",
    season_num: 24,
    order: 8,
    name: "Just Annihilate Them",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_9",
    season_id: "season_24",
    season_num: 24,
    order: 9,
    name: "Go Out With a Bang",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_10",
    season_id: "season_24",
    season_num: 24,
    order: 10,
    name: "I'm No Dummy",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_11",
    season_id: "season_24",
    season_num: 24,
    order: 11,
    name: "Never Say Die",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_12",
    season_id: "season_24",
    season_num: 24,
    order: 12,
    name: "It's Gonna Be Chaos",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_13",
    season_id: "season_24",
    season_num: 24,
    order: 13,
    name: "It's Human Nature",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_14",
    season_id: "season_24",
    season_num: 24,
    order: 14,
    name: "Perception is Not Always Reality",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_15",
    season_id: "season_24",
    season_num: 24,
    order: 15,
    name: "Reunion",
    post_merge: false,
    finale: true,
    merge_occurs: false,
  },
] satisfies Episode<SeasonNumber>[];

export const SEASON_24_CHALLENGES = {
  challenge_0: {
    id: "challenge_0",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 0,
    winning_castaways: [
      "US0356",
      "US0357",
      "US0359",
      "US0360",
      "US0361",
      "US0362",
      "US0363",
      "US0364",
      "US0366",
    ],
  },
  challenge_1: {
    id: "challenge_1",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 1,
    winning_castaways: [
      "US0356",
      "US0357",
      "US0359",
      "US0360",
      "US0361",
      "US0362",
      "US0363",
      "US0364",
      "US0366",
    ],
  },
  challenge_2: {
    id: "challenge_2",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 2,
    winning_castaways: [
      "US0356",
      "US0357",
      "US0359",
      "US0360",
      "US0361",
      "US0362",
      "US0363",
      "US0364",
      "US0366",
    ],
  },
  challenge_3: {
    id: "challenge_3",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 3,
    winning_castaways: [
      "US0356",
      "US0357",
      "US0359",
      "US0360",
      "US0361",
      "US0362",
      "US0363",
      "US0364",
      "US0366",
    ],
  },
  challenge_4: {
    id: "challenge_4",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 4,
    winning_castaways: [
      "US0358",
      "US0365",
      "US0367",
      "US0368",
      "US0369",
      "US0370",
      "US0371",
    ],
  },
  challenge_5: {
    id: "challenge_5",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 5,
    winning_castaways: [
      "US0358",
      "US0365",
      "US0367",
      "US0368",
      "US0369",
      "US0370",
      "US0371",
    ],
  },
  challenge_6: {
    id: "challenge_6",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_reward",
    order: 6,
    winning_castaways: [
      "US0358",
      "US0365",
      "US0367",
      "US0368",
      "US0369",
      "US0370",
      "US0371",
    ],
  },
  challenge_7: {
    id: "challenge_7",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_immunity",
    order: 7,
    winning_castaways: [
      "US0357",
      "US0359",
      "US0360",
      "US0361",
      "US0362",
      "US0363",
      "US0364",
      "US0366",
    ],
  },
  challenge_8: {
    id: "challenge_8",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_reward",
    order: 8,
    winning_castaways: [
      "US0361",
      "US0362",
      "US0364",
      "US0365",
      "US0369",
      "US0370",
      "US0371",
    ],
  },
  challenge_9: {
    id: "challenge_9",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_immunity",
    order: 9,
    winning_castaways: [
      "US0361",
      "US0362",
      "US0364",
      "US0365",
      "US0369",
      "US0370",
      "US0371",
    ],
  },
  challenge_10: {
    id: "challenge_10",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_reward",
    order: 10,
    winning_castaways: [
      "US0361",
      "US0362",
      "US0364",
      "US0365",
      "US0369",
      "US0370",
      "US0371",
    ],
  },
  challenge_11: {
    id: "challenge_11",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "team_reward",
    order: 11,
    winning_castaways: [
      "US0362",
      "US0364",
      "US0367",
      "US0368",
      "US0369",
      "US0370",
    ],
  },
  challenge_12: {
    id: "challenge_12",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "immunity",
    order: 12,
    winning_castaways: ["US0364"],
  },
  challenge_13: {
    id: "challenge_13",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "team_reward",
    order: 13,
    winning_castaways: ["US0361", "US0363", "US0368", "US0370", "US0371"],
  },
  challenge_14: {
    id: "challenge_14",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "immunity",
    order: 14,
    winning_castaways: ["US0362"],
  },
  challenge_15: {
    id: "challenge_15",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "team_reward",
    order: 15,
    winning_castaways: ["US0362", "US0364", "US0365", "US0366", "US0367"],
  },
  challenge_16: {
    id: "challenge_16",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "immunity",
    order: 16,
    winning_castaways: ["US0369"],
  },
  challenge_17: {
    id: "challenge_17",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "immunity",
    order: 17,
    winning_castaways: ["US0364"],
  },
  challenge_18: {
    id: "challenge_18",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "reward",
    order: 18,
    winning_castaways: ["US0371"],
  },
  challenge_19: {
    id: "challenge_19",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "immunity",
    order: 19,
    winning_castaways: ["US0371"],
  },
  challenge_20: {
    id: "challenge_20",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "reward",
    order: 20,
    winning_castaways: ["US0365"],
  },
  challenge_21: {
    id: "challenge_21",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "immunity",
    order: 21,
    winning_castaways: ["US0371"],
  },
  challenge_22: {
    id: "challenge_22",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "reward",
    order: 22,
    winning_castaways: ["US0369"],
  },
  challenge_23: {
    id: "challenge_23",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "immunity",
    order: 23,
    winning_castaways: ["US0367"],
  },
  challenge_24: {
    id: "challenge_24",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "immunity",
    order: 24,
    winning_castaways: ["US0371"],
  },
  challenge_25: {
    id: "challenge_25",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "immunity",
    order: 25,
    winning_castaways: ["US0371"],
  },
} satisfies Record<Challenge["id"], Challenge<CastawayIdType, SeasonNumber>>;

export const SEASON_24_ELIMINATIONS = {
  elimination_1: {
    id: "elimination_1",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_1",
    episode_num: 1,
    order: 1,
    castaway_id: "US0354",
    variant: "medical",
  },
  elimination_2: {
    id: "elimination_2",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_2",
    episode_num: 2,
    order: 2,
    castaway_id: "US0355",
    variant: "tribal",
  },
  elimination_3: {
    id: "elimination_3",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_3",
    episode_num: 3,
    order: 3,
    castaway_id: "US0356",
    variant: "tribal",
  },
  elimination_4: {
    id: "elimination_4",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_4",
    episode_num: 4,
    order: 4,
    castaway_id: "US0357",
    variant: "tribal",
  },
  elimination_5: {
    id: "elimination_5",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_5",
    episode_num: 5,
    order: 5,
    castaway_id: "US0358",
    variant: "tribal",
  },
  elimination_6: {
    id: "elimination_6",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_6",
    episode_num: 6,
    order: 6,
    castaway_id: "US0359",
    variant: "medical",
  },
  elimination_7: {
    id: "elimination_7",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    order: 7,
    castaway_id: "US0360",
    variant: "tribal",
  },
  elimination_8: {
    id: "elimination_8",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_8",
    episode_num: 8,
    order: 8,
    castaway_id: "US0361",
    variant: "tribal",
  },
  elimination_9: {
    id: "elimination_9",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_9",
    episode_num: 9,
    order: 9,
    castaway_id: "US0362",
    variant: "tribal",
  },
  elimination_10: {
    id: "elimination_10",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_10",
    episode_num: 10,
    order: 10,
    castaway_id: "US0363",
    variant: "tribal",
  },
  elimination_11: {
    id: "elimination_11",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_11",
    episode_num: 11,
    order: 11,
    castaway_id: "US0364",
    variant: "tribal",
  },
  elimination_12: {
    id: "elimination_12",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_12",
    episode_num: 12,
    order: 12,
    castaway_id: "US0365",
    variant: "tribal",
  },
  elimination_13: {
    id: "elimination_13",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_13",
    episode_num: 13,
    order: 13,
    castaway_id: "US0366",
    variant: "tribal",
  },
  elimination_14: {
    id: "elimination_14",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    order: 14,
    castaway_id: "US0367",
    variant: "tribal",
  },
  elimination_15: {
    id: "elimination_15",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    order: 15,
    castaway_id: "US0368",
    variant: "tribal",
  },
  elimination_16: {
    id: "elimination_16",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    order: 16,
    castaway_id: "US0369",
    variant: "final_tribal_council",
  },
  elimination_17: {
    id: "elimination_17",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    order: 17,
    castaway_id: "US0370",
    variant: "final_tribal_council",
  },
} satisfies Record<
  Elimination["id"],
  Elimination<CastawayIdType, SeasonNumber>
>;

export const SEASON_24_EVENTS = {
  event_1: {
    id: "event_1",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_1",
    episode_num: 1,
    castaway_id: "US0370",
    action: "find_idol",
    multiplier: null,
  },
  event_2: {
    id: "event_2",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_1",
    episode_num: 1,
    castaway_id: "US0359",
    action: "win_idol",
    multiplier: null,
  },
  event_3: {
    id: "event_3",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_5",
    episode_num: 5,
    castaway_id: "US0371",
    action: "find_idol",
    multiplier: null,
  },
  event_4: {
    id: "event_4",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0364",
    action: "find_idol",
    multiplier: null,
  },
  event_5: {
    id: "event_5",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_9",
    episode_num: 9,
    castaway_id: "US0364",
    action: "use_idol",
    multiplier: null,
  },
  event_6: {
    id: "event_6",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_9",
    episode_num: 9,
    castaway_id: "US0364",
    action: "votes_negated_by_idol",
    multiplier: 2,
  },
  event_7: {
    id: "event_7",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0360",
    action: "make_merge",
    multiplier: null,
  },
  event_8: {
    id: "event_8",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0361",
    action: "make_merge",
    multiplier: null,
  },
  event_9: {
    id: "event_9",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0362",
    action: "make_merge",
    multiplier: null,
  },
  event_10: {
    id: "event_10",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0363",
    action: "make_merge",
    multiplier: null,
  },
  event_11: {
    id: "event_11",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0364",
    action: "make_merge",
    multiplier: null,
  },
  event_12: {
    id: "event_12",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0365",
    action: "make_merge",
    multiplier: null,
  },
  event_13: {
    id: "event_13",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0366",
    action: "make_merge",
    multiplier: null,
  },
  event_14: {
    id: "event_14",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0367",
    action: "make_merge",
    multiplier: null,
  },
  event_15: {
    id: "event_15",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0368",
    action: "make_merge",
    multiplier: null,
  },
  event_16: {
    id: "event_16",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0369",
    action: "make_merge",
    multiplier: null,
  },
  event_17: {
    id: "event_17",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0370",
    action: "make_merge",
    multiplier: null,
  },
  event_18: {
    id: "event_18",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0371",
    action: "make_merge",
    multiplier: null,
  },
  event_19: {
    id: "event_19",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0371",
    action: "win_survivor",
    multiplier: null,
  },
  event_20: {
    id: "event_20",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0369",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_21: {
    id: "event_21",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0370",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_22: {
    id: "event_22",
    season_id: "season_24",
    season_num: 24,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0371",
    action: "make_final_tribal_council",
    multiplier: null,
  },
} satisfies Record<GameEvent["id"], GameEvent<CastawayIdType, SeasonNumber>>;
