import {
  CastawayLookup,
  Challenge,
  Elimination,
  Episode,
  GameEvent,
  Player,
} from "../../types";

// eslint-disable-next-line @typescript-eslint/no-unused-vars -- used only in typeof for type derivation
const CastawayIds = [
  "US0744",
  "US0735",
  "US0736",
  "US0739",
  "US0741",
  "US0737",
  "US0748",
  "US0743",
  "US0742",
  "US0734",
  "US0738",
  "US0750",
  "US0751",
  "US0740",
  "US0745",
  "US0746",
  "US0749",
  "US0747",
] as const;

type CastawayIdType = (typeof CastawayIds)[number];

type SeasonNumber = 49;

const buildPlayer = <T extends CastawayIdType>(
  p: { castaway_id: T; full_name: string; img: string } & Partial<
    Omit<
      Player<T, SeasonNumber>,
      "season_id" | "season_num" | "castaway_id" | "full_name" | "img"
    >
  >,
): Player<T, SeasonNumber> => ({
  ...p,
  season_num: 49,
  season_id: "season_49",
});

export const SEASON_49_CASTAWAY_LOOKUP: CastawayLookup = {
  US0744: { full_name: "Nicole Mazullo", castaway: "Nicole" },
  US0735: { full_name: "Annie Davis", castaway: "Annie" },
  US0736: { full_name: "Jake Latimer", castaway: "Jake" },
  US0739: { full_name: "Jeremiah Ing", castaway: "Jeremiah" },
  US0741: { full_name: "Matt Williams", castaway: "Matt" },
  US0737: { full_name: "Jason Treul", castaway: "Jason" },
  US0748: { full_name: "Shannon Fairweather", castaway: "Shannon" },
  US0743: { full_name: "Nate Moore", castaway: "Nate" },
  US0742: { full_name: "MC Chukwujekwu", castaway: "MC" },
  US0734: { full_name: "Alex Moore", castaway: "Alex" },
  US0738: { full_name: "Jawan Pitts", castaway: "Jawan" },
  US0750: { full_name: "Sophie Segreti", castaway: "Sophie" },
  US0751: { full_name: "Steven Ramm", castaway: "Steven" },
  US0740: { full_name: "Kristina Mills", castaway: "Kristina" },
  US0745: { full_name: "Rizo Velovic", castaway: "Rizo" },
  US0746: { full_name: "Sage Ahrens-Nichols", castaway: "Sage" },
  US0749: { full_name: "Sophi Balerdi", castaway: "Sophi/Soph" },
  US0747: { full_name: "Savannah Louie", castaway: "Savannah" },
};

export const SEASON_49_PLAYERS = [
  buildPlayer({
    castaway_id: "US0744",
    full_name: "Nicole Mazullo",
    img: "/images/season_49/Nicole-Mazullo.jpg",
    description:
      "Age: 26 | Hometown: Philadelphia, Pennsylvania | Occupation: Financial Crime Consultant",
    age: 26,
    profession: "Financial Crime Consultant",
    hometown: "Philadelphia, Pennsylvania",
  }),
  buildPlayer({
    castaway_id: "US0735",
    full_name: "Annie Davis",
    img: "/images/season_49/Annie-Davis.jpg",
    description: "Age: 49 | Hometown: Austin, Texas | Occupation: Musician",
    age: 49,
    profession: "Musician",
    hometown: "Austin, Texas",
    nickname: "Annie",
  }),
  buildPlayer({
    castaway_id: "US0736",
    full_name: "Jake Latimer",
    img: "/images/season_49/Jake-Latimer.jpg",
    description:
      "Age: 35 | Hometown: St. Albert, Alberta | Occupation: Correctional Officer",
    age: 35,
    profession: "Correctional Officer",
    hometown: "St. Albert, Alberta",
  }),
  buildPlayer({
    castaway_id: "US0739",
    full_name: "Jeremiah Ing",
    img: "/images/season_49/Jeremiah-Ing.jpg",
    description:
      "Age: 38 | Hometown: Toronto, Ontario | Occupation: Global Events Manager",
    age: 38,
    profession: "Global Events Manager",
    hometown: "Toronto, Ontario",
  }),
  buildPlayer({
    castaway_id: "US0741",
    full_name: "Matt Williams",
    img: "/images/season_49/Matt-Williams.jpg",
    description:
      "Age: 52 | Hometown: St. George, Utah | Occupation: Airport Ramp Agent",
    age: 52,
    profession: "Airport Ramp Agent",
    hometown: "St. George, Utah",
  }),
  buildPlayer({
    castaway_id: "US0737",
    full_name: "Jason Treul",
    img: "/images/season_49/Jason-Treul.jpg",
    description:
      "Age: 32 | Hometown: Santa Ana, California | Occupation: Law Clerk",
    age: 32,
    profession: "Law Clerk",
    hometown: "Santa Ana, California",
  }),
  buildPlayer({
    castaway_id: "US0748",
    full_name: "Shannon Fairweather",
    img: "/images/season_49/Shannon-Fairweather.jpg",
    description:
      "Age: 27 | Hometown: Boston, Massachusetts | Occupation: Wellness Specialist",
    age: 27,
    profession: "Wellness Specialist",
    hometown: "Boston, Massachusetts",
  }),
  buildPlayer({
    castaway_id: "US0743",
    full_name: "Nate Moore",
    img: "/images/season_49/Nate-Moore.jpg",
    description:
      "Age: 47 | Hometown: Hermosa Beach, California | Occupation: Film Producer",
    age: 47,
    profession: "Film Producer",
    hometown: "Hermosa Beach, California",
  }),
  buildPlayer({
    castaway_id: "US0742",
    full_name: "MC Chukwujekwu",
    img: "/images/season_49/MC-Chukwujekwu.jpg",
    description:
      "Age: 29 | Hometown: San Diego, California | Occupation: Fitness Trainer",
    age: 29,
    profession: "Fitness Trainer",
    hometown: "San Diego, California",
    nickname: "MC",
  }),
  buildPlayer({
    castaway_id: "US0734",
    full_name: "Alex Moore",
    img: "/images/season_49/Alex-Moore.jpg",
    description:
      "Age: 26 | Hometown: Washington, District of Columbia | Occupation: Political Communications Director",
    age: 26,
    profession: "Political Communications Director",
    hometown: "Washington, District of Columbia",
  }),
  buildPlayer({
    castaway_id: "US0738",
    full_name: "Jawan Pitts",
    img: "/images/season_49/Jawan-Pitts.jpg",
    description:
      "Age: 28 | Hometown: Los Angeles, California | Occupation: Video Editor",
    age: 28,
    profession: "Video Editor",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0750",
    full_name: "Sophie Segreti",
    img: "/images/season_49/Sophie-Segreti.jpg",
    description:
      "Age: 31 | Hometown: New York, New York | Occupation: Strategy Associate",
    age: 31,
    profession: "Strategy Associate",
    hometown: "New York, New York",
    nickname: "Sophie",
  }),
  buildPlayer({
    castaway_id: "US0751",
    full_name: "Steven Ramm",
    img: "/images/season_49/Steven-Ramm.jpg",
    description:
      "Age: 35 | Hometown: Denver, Colorado | Occupation: Rocket Scientist",
    age: 35,
    profession: "Rocket Scientist",
    hometown: "Denver, Colorado",
  }),
  buildPlayer({
    castaway_id: "US0740",
    full_name: "Kristina Mills",
    img: "/images/season_49/Kristina-Mills.jpg",
    description:
      "Age: 35 | Hometown: Edmond, Oklahoma | Occupation: MBA Career Coach",
    age: 35,
    profession: "MBA Career Coach",
    hometown: "Edmond, Oklahoma",
  }),
  buildPlayer({
    castaway_id: "US0745",
    full_name: "Rizo Velovic",
    img: "/images/season_49/Rizo-Velovic.jpg",
    description:
      "Age: 25 | Hometown: Yonkers, New York | Occupation: Tech Sales",
    age: 25,
    profession: "Tech Sales",
    hometown: "Yonkers, New York",
  }),
  buildPlayer({
    castaway_id: "US0746",
    full_name: "Sage Ahrens-Nichols",
    img: "/images/season_49/Sage-Ahrens-Nichols.jpg",
    description:
      "Age: 30 | Hometown: Olympia, Washington | Occupation: Clinical Social Worker",
    age: 30,
    profession: "Clinical Social Worker",
    hometown: "Olympia, Washington",
  }),
  buildPlayer({
    castaway_id: "US0749",
    full_name: "Sophi Balerdi",
    img: "/images/season_49/Sophi-Balerdi.jpg",
    description:
      "Age: 27 | Hometown: Miami, Florida | Occupation: Entrepreneur",
    age: 27,
    profession: "Entrepreneur",
    hometown: "Miami, Florida",
    nickname: "Sophi/Soph",
  }),
  buildPlayer({
    castaway_id: "US0747",
    full_name: "Savannah Louie",
    img: "/images/season_49/Savannah-Louie.jpg",
    description:
      "Age: 31 | Hometown: Atlanta, Georgia | Occupation: Former Reporter",
    age: 31,
    profession: "Former Reporter",
    hometown: "Atlanta, Georgia",
  }),
] satisfies Player<CastawayIdType, SeasonNumber>[];

export const SEASON_49_EPISODES = [
  {
    id: "episode_1",
    season_id: "season_49",
    season_num: 49,
    order: 1,
    name: "Act One of a Horror Film",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_2",
    season_id: "season_49",
    season_num: 49,
    order: 2,
    name: "Cinema",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_3",
    season_id: "season_49",
    season_num: 49,
    order: 3,
    name: "Lovable Losers",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_4",
    season_id: "season_49",
    season_num: 49,
    order: 4,
    name: "Go Kick Rocks, Bro",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_5",
    season_id: "season_49",
    season_num: 49,
    order: 5,
    name: "I'm a Wolf, Baby",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_6",
    season_id: "season_49",
    season_num: 49,
    order: 6,
    name: "The Devil's Shoes",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_7",
    season_id: "season_49",
    season_num: 49,
    order: 7,
    name: "Blood Will Be Drawn",
    post_merge: true,
    finale: false,
    merge_occurs: true,
  },
  {
    id: "episode_8",
    season_id: "season_49",
    season_num: 49,
    order: 8,
    name: "Hot Grim Reaper",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_9",
    season_id: "season_49",
    season_num: 49,
    order: 9,
    name: "If You're Loyal to All, You're Loyal to None",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_10",
    season_id: "season_49",
    season_num: 49,
    order: 10,
    name: "Huge Dose of Bamboozle",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_11",
    season_id: "season_49",
    season_num: 49,
    order: 11,
    name: "Cherry On Top",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_12",
    season_id: "season_49",
    season_num: 49,
    order: 12,
    name: "The Die Is Cast",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_13",
    season_id: "season_49",
    season_num: 49,
    order: 13,
    name: "A Fever Dream",
    post_merge: true,
    finale: true,
    merge_occurs: false,
  },
] satisfies Episode<SeasonNumber>[];

export const SEASON_49_CHALLENGES = {
  challenge_0: {
    id: "challenge_0",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 0,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_1: {
    id: "challenge_1",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 1,
    winning_castaways: ["US0745"],
  },
  challenge_2: {
    id: "challenge_2",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 2,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_3: {
    id: "challenge_3",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 3,
    winning_castaways: [
      "US0738",
      "US0743",
      "US0745",
      "US0746",
      "US0747",
      "US0748",
    ],
  },
  challenge_4: {
    id: "challenge_4",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 4,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_5: {
    id: "challenge_5",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 5,
    winning_castaways: [
      "US0738",
      "US0743",
      "US0745",
      "US0746",
      "US0747",
      "US0748",
    ],
  },
  challenge_6: {
    id: "challenge_6",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 6,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_7: {
    id: "challenge_7",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 7,
    winning_castaways: [
      "US0738",
      "US0743",
      "US0745",
      "US0746",
      "US0747",
      "US0748",
    ],
  },
  challenge_8: {
    id: "challenge_8",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 8,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_9: {
    id: "challenge_9",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 9,
    winning_castaways: [
      "US0738",
      "US0743",
      "US0745",
      "US0746",
      "US0747",
      "US0748",
    ],
  },
  challenge_10: {
    id: "challenge_10",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 10,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_11: {
    id: "challenge_11",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 11,
    winning_castaways: [
      "US0738",
      "US0743",
      "US0745",
      "US0746",
      "US0747",
      "US0748",
    ],
  },
  challenge_12: {
    id: "challenge_12",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 12,
    winning_castaways: [
      "US0737",
      "US0740",
      "US0741",
      "US0742",
      "US0750",
      "US0751",
    ],
  },
  challenge_13: {
    id: "challenge_13",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 13,
    winning_castaways: [
      "US0738",
      "US0743",
      "US0745",
      "US0746",
      "US0747",
      "US0748",
    ],
  },
  challenge_14: {
    id: "challenge_14",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_immunity",
    order: 14,
    winning_castaways: [
      "US0734",
      "US0740",
      "US0742",
      "US0746",
      "US0748",
      "US0750",
      "US0751",
    ],
  },
  challenge_15: {
    id: "challenge_15",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_reward",
    order: 15,
    winning_castaways: [
      "US0734",
      "US0740",
      "US0742",
      "US0746",
      "US0748",
      "US0750",
      "US0751",
    ],
  },
  challenge_16: {
    id: "challenge_16",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_immunity",
    order: 16,
    winning_castaways: [
      "US0734",
      "US0740",
      "US0742",
      "US0746",
      "US0748",
      "US0750",
      "US0751",
    ],
  },
  challenge_17: {
    id: "challenge_17",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_reward",
    order: 17,
    winning_castaways: [
      "US0734",
      "US0740",
      "US0742",
      "US0746",
      "US0748",
      "US0750",
      "US0751",
    ],
  },
  challenge_18: {
    id: "challenge_18",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_reward",
    order: 18,
    winning_castaways: ["US0734", "US0740", "US0743", "US0750"],
  },
  challenge_19: {
    id: "challenge_19",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_immunity",
    order: 19,
    winning_castaways: ["US0742", "US0745", "US0747", "US0749"],
  },
  challenge_20: {
    id: "challenge_20",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_immunity",
    order: 20,
    winning_castaways: ["US0734", "US0740", "US0743", "US0750"],
  },
  challenge_21: {
    id: "challenge_21",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "team_reward",
    order: 21,
    winning_castaways: ["US0743", "US0745", "US0749", "US0750", "US0751"],
  },
  challenge_22: {
    id: "challenge_22",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "immunity",
    order: 22,
    winning_castaways: ["US0750"],
  },
  challenge_23: {
    id: "challenge_23",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "team_immunity",
    order: 23,
    winning_castaways: ["US0734", "US0740", "US0749", "US0751"],
  },
  challenge_24: {
    id: "challenge_24",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "immunity",
    order: 24,
    winning_castaways: ["US0747"],
  },
  challenge_25: {
    id: "challenge_25",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "team_reward",
    order: 25,
    winning_castaways: ["US0734", "US0740", "US0747", "US0749", "US0751"],
  },
  challenge_26: {
    id: "challenge_26",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "team_reward",
    order: 26,
    winning_castaways: ["US0734", "US0740", "US0747", "US0750"],
  },
  challenge_27: {
    id: "challenge_27",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "immunity",
    order: 27,
    winning_castaways: ["US0747", "US0751"],
  },
  challenge_28: {
    id: "challenge_28",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "team_reward",
    order: 28,
    winning_castaways: ["US0746", "US0749", "US0750", "US0751"],
  },
  challenge_29: {
    id: "challenge_29",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "immunity",
    order: 29,
    winning_castaways: ["US0750"],
  },
  challenge_30: {
    id: "challenge_30",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "immunity",
    order: 30,
    winning_castaways: ["US0751"],
  },
  challenge_31: {
    id: "challenge_31",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "reward",
    order: 31,
    winning_castaways: ["US0751"],
  },
  challenge_32: {
    id: "challenge_32",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "reward",
    order: 32,
    winning_castaways: ["US0747"],
  },
  challenge_33: {
    id: "challenge_33",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "immunity",
    order: 33,
    winning_castaways: ["US0747"],
  },
  challenge_34: {
    id: "challenge_34",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "immunity",
    order: 34,
    winning_castaways: ["US0747"],
  },
  challenge_35: {
    id: "challenge_35",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "reward",
    order: 35,
    winning_castaways: ["US0747"],
  },
  challenge_36: {
    id: "challenge_36",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "immunity",
    order: 36,
    winning_castaways: ["US0749"],
  },
} satisfies Record<Challenge["id"], Challenge<CastawayIdType, SeasonNumber>>;

export const SEASON_49_ELIMINATIONS = {
  elimination_1: {
    id: "elimination_1",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_1",
    episode_num: 1,
    order: 1,
    castaway_id: "US0744",
    variant: "tribal",
  },
  elimination_2: {
    id: "elimination_2",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    order: 2,
    castaway_id: "US0735",
    variant: "tribal",
  },
  elimination_3: {
    id: "elimination_3",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    order: 3,
    castaway_id: "US0736",
    variant: "medical",
  },
  elimination_4: {
    id: "elimination_4",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    order: 4,
    castaway_id: "US0739",
    variant: "tribal",
  },
  elimination_5: {
    id: "elimination_5",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_4",
    episode_num: 4,
    order: 5,
    castaway_id: "US0741",
    variant: "tribal",
  },
  elimination_6: {
    id: "elimination_6",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_5",
    episode_num: 5,
    order: 6,
    castaway_id: "US0737",
    variant: "tribal",
  },
  elimination_7: {
    id: "elimination_7",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_6",
    episode_num: 6,
    order: 7,
    castaway_id: "US0748",
    variant: "tribal",
  },
  elimination_8: {
    id: "elimination_8",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    order: 8,
    castaway_id: "US0743",
    variant: "tribal",
  },
  elimination_9: {
    id: "elimination_9",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_8",
    episode_num: 8,
    order: 9,
    castaway_id: "US0742",
    variant: "tribal",
  },
  elimination_10: {
    id: "elimination_10",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_9",
    episode_num: 9,
    order: 10,
    castaway_id: "US0734",
    variant: "tribal",
  },
  elimination_11: {
    id: "elimination_11",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_10",
    episode_num: 10,
    order: 11,
    castaway_id: "US0738",
    variant: "tribal",
  },
  elimination_12: {
    id: "elimination_12",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_11",
    episode_num: 11,
    order: 12,
    castaway_id: "US0750",
    variant: "tribal",
  },
  elimination_13: {
    id: "elimination_13",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_12",
    episode_num: 12,
    order: 13,
    castaway_id: "US0751",
    variant: "tribal",
  },
  elimination_14: {
    id: "elimination_14",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    order: 14,
    castaway_id: "US0740",
    variant: "tribal",
  },
  elimination_15: {
    id: "elimination_15",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    order: 15,
    castaway_id: "US0745",
    variant: "tribal",
  },
  elimination_16: {
    id: "elimination_16",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    order: 16,
    castaway_id: "US0746",
    variant: "final_tribal_council",
  },
  elimination_17: {
    id: "elimination_17",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    order: 17,
    castaway_id: "US0749",
    variant: "final_tribal_council",
  },
} satisfies Record<
  Elimination["id"],
  Elimination<CastawayIdType, SeasonNumber>
>;

export const SEASON_49_EVENTS = {
  event_1: {
    id: "event_1",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0734",
    action: "find_beware_advantage",
    multiplier: null,
  },
  event_2: {
    id: "event_2",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0734",
    action: "accept_beware_advantage",
    multiplier: null,
  },
  event_3: {
    id: "event_3",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0734",
    action: "fulfill_beware_advantage",
    multiplier: null,
  },
  event_4: {
    id: "event_4",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0734",
    action: "find_idol",
    multiplier: null,
  },
  event_5: {
    id: "event_5",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0734",
    action: "use_idol",
    multiplier: null,
  },
  event_6: {
    id: "event_6",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0738",
    action: "find_challenge_advantage",
    multiplier: null,
  },
  event_7: {
    id: "event_7",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0738",
    action: "use_challenge_advantage",
    multiplier: null,
  },
  event_8: {
    id: "event_8",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0742",
    action: "find_beware_advantage",
    multiplier: null,
  },
  event_9: {
    id: "event_9",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0742",
    action: "accept_beware_advantage",
    multiplier: null,
  },
  event_10: {
    id: "event_10",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0742",
    action: "fulfill_beware_advantage",
    multiplier: null,
  },
  event_11: {
    id: "event_11",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0742",
    action: "find_idol",
    multiplier: null,
  },
  event_12: {
    id: "event_12",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0742",
    action: "use_idol",
    multiplier: null,
  },
  event_13: {
    id: "event_13",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0745",
    action: "find_beware_advantage",
    multiplier: null,
  },
  event_14: {
    id: "event_14",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0745",
    action: "accept_beware_advantage",
    multiplier: null,
  },
  event_15: {
    id: "event_15",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_4",
    episode_num: 4,
    castaway_id: "US0745",
    action: "fulfill_beware_advantage",
    multiplier: null,
  },
  event_16: {
    id: "event_16",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_4",
    episode_num: 4,
    castaway_id: "US0745",
    action: "find_idol",
    multiplier: null,
  },
  event_17: {
    id: "event_17",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0745",
    action: "use_idol",
    multiplier: null,
  },
  event_18: {
    id: "event_18",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_6",
    episode_num: 6,
    castaway_id: "US0749",
    action: "find_knowledge_is_power",
    multiplier: null,
  },
  event_19: {
    id: "event_19",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_12",
    episode_num: 12,
    castaway_id: "US0749",
    action: "use_knowledge_is_power",
    multiplier: null,
  },
  event_20: {
    id: "event_20",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0746",
    action: "find_challenge_advantage",
    multiplier: null,
  },
  event_21: {
    id: "event_21",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0746",
    action: "use_challenge_advantage",
    multiplier: null,
  },
  event_22: {
    id: "event_22",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0747",
    action: "find_bank_your_vote",
    multiplier: null,
  },
  event_23: {
    id: "event_23",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0747",
    action: "use_bank_your_vote",
    multiplier: null,
  },
  event_24: {
    id: "event_24",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0740",
    action: "find_idol",
    multiplier: null,
  },
  event_25: {
    id: "event_25",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0740",
    action: "use_idol",
    multiplier: null,
  },
  event_26: {
    id: "event_26",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_11",
    episode_num: 11,
    castaway_id: "US0751",
    action: "find_block_a_vote",
    multiplier: null,
  },
  event_27: {
    id: "event_27",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_12",
    episode_num: 12,
    castaway_id: "US0751",
    action: "use_block_a_vote",
    multiplier: null,
  },
  event_28: {
    id: "event_28",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0749",
    action: "find_challenge_advantage",
    multiplier: null,
  },
  event_29: {
    id: "event_29",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0749",
    action: "use_challenge_advantage",
    multiplier: null,
  },
  event_30: {
    id: "event_30",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0734",
    action: "votes_negated_by_idol",
    multiplier: 1,
  },
  event_31: {
    id: "event_31",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0740",
    action: "votes_negated_by_idol",
    multiplier: 1,
  },
  event_32: {
    id: "event_32",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0736",
    action: "go_on_journey",
    multiplier: null,
  },
  event_33: {
    id: "event_33",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0738",
    action: "go_on_journey",
    multiplier: null,
  },
  event_34: {
    id: "event_34",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0738",
    action: "journey_risked_vote",
    multiplier: null,
  },
  event_35: {
    id: "event_35",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0738",
    action: "journey_won_game",
    multiplier: null,
  },
  event_36: {
    id: "event_36",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0738",
    action: "win_other_advantage",
    multiplier: null,
  },
  event_37: {
    id: "event_37",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_2",
    episode_num: 2,
    castaway_id: "US0741",
    action: "go_on_journey",
    multiplier: null,
  },
  event_38: {
    id: "event_38",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_5",
    episode_num: 5,
    castaway_id: "US0742",
    action: "go_on_journey",
    multiplier: null,
  },
  event_39: {
    id: "event_39",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_5",
    episode_num: 5,
    castaway_id: "US0743",
    action: "go_on_journey",
    multiplier: null,
  },
  event_40: {
    id: "event_40",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_11",
    episode_num: 11,
    castaway_id: "US0751",
    action: "go_on_journey",
    multiplier: null,
  },
  event_41: {
    id: "event_41",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0747",
    action: "win_fire_making",
    multiplier: null,
  },
  event_42: {
    id: "event_42",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0734",
    action: "make_merge",
    multiplier: null,
  },
  event_43: {
    id: "event_43",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0738",
    action: "make_merge",
    multiplier: null,
  },
  event_44: {
    id: "event_44",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0740",
    action: "make_merge",
    multiplier: null,
  },
  event_45: {
    id: "event_45",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0742",
    action: "make_merge",
    multiplier: null,
  },
  event_46: {
    id: "event_46",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0743",
    action: "make_merge",
    multiplier: null,
  },
  event_47: {
    id: "event_47",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0745",
    action: "make_merge",
    multiplier: null,
  },
  event_48: {
    id: "event_48",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0746",
    action: "make_merge",
    multiplier: null,
  },
  event_49: {
    id: "event_49",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0747",
    action: "make_merge",
    multiplier: null,
  },
  event_50: {
    id: "event_50",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0749",
    action: "make_merge",
    multiplier: null,
  },
  event_51: {
    id: "event_51",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0750",
    action: "make_merge",
    multiplier: null,
  },
  event_52: {
    id: "event_52",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0751",
    action: "make_merge",
    multiplier: null,
  },
  event_53: {
    id: "event_53",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0747",
    action: "win_survivor",
    multiplier: null,
  },
  event_54: {
    id: "event_54",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0746",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_55: {
    id: "event_55",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0749",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_56: {
    id: "event_56",
    season_id: "season_49",
    season_num: 49,
    episode_id: "episode_13",
    episode_num: 13,
    castaway_id: "US0747",
    action: "make_final_tribal_council",
    multiplier: null,
  },
} satisfies Record<GameEvent["id"], GameEvent<CastawayIdType, SeasonNumber>>;
