import {
  CastawayLookup,
  Challenge,
  Elimination,
  Episode,
  GameEvent,
  Player,
} from "../../types";

// eslint-disable-next-line @typescript-eslint/no-unused-vars -- used only in typeof for type derivation
const CastawayIds = [
  "US0461",
  "US0462",
  "US0463",
  "US0464",
  "US0465",
  "US0466",
  "US0467",
  "US0468",
  "US0469",
  "US0470",
  "US0471",
  "US0472",
  "US0473",
  "US0474",
  "US0475",
  "US0476",
  "US0477",
  "US0478",
] as const;

type CastawayIdType = (typeof CastawayIds)[number];

type SeasonNumber = 32;

const buildPlayer = <T extends CastawayIdType>(
  p: { castaway_id: T; full_name: string; img: string } & Partial<
    Omit<
      Player<T, SeasonNumber>,
      "season_id" | "season_num" | "castaway_id" | "full_name" | "img"
    >
  >,
): Player<T, SeasonNumber> => ({
  ...p,
  season_num: 32,
  season_id: "season_32",
});

export const SEASON_32_CASTAWAY_LOOKUP: CastawayLookup = {
  US0461: { full_name: "Darnell Hamilton", castaway: "Darnell" },
  US0462: { full_name: "Jennifer Lanzetti", castaway: "Jennifer" },
  US0463: { full_name: "Elisabeth Markham", castaway: "Liz" },
  US0464: { full_name: "Caleb Reynolds", castaway: "Caleb" },
  US0465: { full_name: "Alecia Holden", castaway: "Alecia" },
  US0466: { full_name: "Anna Khait", castaway: "Anna" },
  US0467: { full_name: "Peter Baggenstos", castaway: "Peter" },
  US0468: { full_name: "Neal Gottlieb", castaway: "Neal" },
  US0469: { full_name: "Nick Maiorano", castaway: "Nick" },
  US0470: { full_name: "Debbie Wanner", castaway: "Debbie" },
  US0471: { full_name: "Scot Pollard", castaway: "Scot" },
  US0472: { full_name: "Julia Sokolowski", castaway: "Julia" },
  US0473: { full_name: "Kyle Jason", castaway: "Jason" },
  US0474: { full_name: "Joseph Del Campo", castaway: "Joe" },
  US0475: { full_name: "Cydney Gillon", castaway: "Cydney" },
  US0476: { full_name: "Tai Trang", castaway: "Tai" },
  US0477: { full_name: "Aubry Bracco", castaway: "Aubry" },
  US0478: { full_name: "Michele Fitzgerald", castaway: "Michele" },
};

export const SEASON_32_PLAYERS = [
  buildPlayer({
    castaway_id: "US0461",
    full_name: "Darnell Hamilton",
    img: "/images/season_32/Darnell-Hamilton.jpg",
    description:
      "Age: 27 | Hometown: Chicago, Illinois | Occupation: Postal Worker",
    age: 27,
    profession: "Postal Worker",
    hometown: "Chicago, Illinois",
  }),
  buildPlayer({
    castaway_id: "US0462",
    full_name: "Jennifer Lanzetti",
    img: "/images/season_32/Jennifer-Lanzetti.jpg",
    description:
      "Age: 38 | Hometown: Salt Lake City, Utah | Occupation: Contractor",
    age: 38,
    profession: "Contractor",
    hometown: "Salt Lake City, Utah",
  }),
  buildPlayer({
    castaway_id: "US0463",
    full_name: "Elisabeth Markham",
    img: "/images/season_32/Elisabeth-Markham.jpg",
    description:
      "Age: 27 | Hometown: New York City, New York | Occupation: Quantitative Strategist",
    age: 27,
    profession: "Quantitative Strategist",
    hometown: "New York City, New York",
    nickname: "Liz",
  }),
  buildPlayer({
    castaway_id: "US0464",
    full_name: "Caleb Reynolds",
    img: "/images/season_32/Caleb-Reynolds.jpg",
    description:
      "Age: 27 | Hometown: Hopkinsville, Kentucky | Occupation: Army Veteran",
    age: 27,
    profession: "Army Veteran",
    hometown: "Hopkinsville, Kentucky",
  }),
  buildPlayer({
    castaway_id: "US0465",
    full_name: "Alecia Holden",
    img: "/images/season_32/Alecia-Holden.jpg",
    description:
      "Age: 24 | Hometown: Dallas, Texas | Occupation: Real Estate Agent",
    age: 24,
    profession: "Real Estate Agent",
    hometown: "Dallas, Texas",
  }),
  buildPlayer({
    castaway_id: "US0466",
    full_name: "Anna Khait",
    img: "/images/season_32/Anna-Khait.jpg",
    description:
      "Age: 26 | Hometown: Brooklyn, New York | Occupation: Pro Poker Player",
    age: 26,
    profession: "Pro Poker Player",
    hometown: "Brooklyn, New York",
  }),
  buildPlayer({
    castaway_id: "US0467",
    full_name: "Peter Baggenstos",
    img: "/images/season_32/Peter-Baggenstos.jpg",
    description:
      "Age: 34 | Hometown: Minneapolis, Minnesota | Occupation: ER Doctor",
    age: 34,
    profession: "ER Doctor",
    hometown: "Minneapolis, Minnesota",
  }),
  buildPlayer({
    castaway_id: "US0468",
    full_name: "Neal Gottlieb",
    img: "/images/season_32/Neal-Gottlieb.jpg",
    description:
      "Age: 38 | Hometown: Sausalito, California | Occupation: Ice Cream Entrepreneur",
    age: 38,
    profession: "Ice Cream Entrepreneur",
    hometown: "Sausalito, California",
  }),
  buildPlayer({
    castaway_id: "US0469",
    full_name: "Nick Maiorano",
    img: "/images/season_32/Nick-Maiorano.jpg",
    description:
      "Age: 30 | Hometown: Redondo Beach, California | Occupation: Personal Trainer",
    age: 30,
    profession: "Personal Trainer",
    hometown: "Redondo Beach, California",
  }),
  buildPlayer({
    castaway_id: "US0470",
    full_name: "Debbie Wanner",
    img: "/images/season_32/Debbie-Wanner.jpg",
    description:
      "Age: 49 | Hometown: Reading, Pennsylvania | Occupation: Chemist",
    age: 49,
    profession: "Chemist",
    hometown: "Reading, Pennsylvania",
  }),
  buildPlayer({
    castaway_id: "US0471",
    full_name: "Scot Pollard",
    img: "/images/season_32/Scot-Pollard.jpg",
    description:
      "Age: 40 | Hometown: Carmel, Indiana | Occupation: Former NBA Champion",
    age: 40,
    profession: "Former NBA Champion",
    hometown: "Carmel, Indiana",
  }),
  buildPlayer({
    castaway_id: "US0472",
    full_name: "Julia Sokolowski",
    img: "/images/season_32/Julia-Sokolowski.jpg",
    description:
      "Age: 19 | Hometown: Boston, Massachusetts | Occupation: College Student",
    age: 19,
    profession: "College Student",
    hometown: "Boston, Massachusetts",
  }),
  buildPlayer({
    castaway_id: "US0473",
    full_name: "Kyle Jason",
    img: "/images/season_32/Kyle-Jason.jpg",
    description:
      "Age: 31 | Hometown: Detroit, Michigan | Occupation: Bounty Hunter",
    age: 31,
    profession: "Bounty Hunter",
    hometown: "Detroit, Michigan",
    nickname: "Jason",
  }),
  buildPlayer({
    castaway_id: "US0474",
    full_name: "Joseph Del Campo",
    img: "/images/season_32/Joseph-Del-Campo.jpg",
    description:
      "Age: 71 | Hometown: Vero Beach, Florida | Occupation: Former FBI Agent",
    age: 71,
    profession: "Former FBI Agent",
    hometown: "Vero Beach, Florida",
    nickname: "Joe",
  }),
  buildPlayer({
    castaway_id: "US0475",
    full_name: "Cydney Gillon",
    img: "/images/season_32/Cydney-Gillon.jpg",
    description:
      "Age: 22 | Hometown: Douglasville, Georgia | Occupation: Bodybuilder",
    age: 22,
    profession: "Bodybuilder",
    hometown: "Douglasville, Georgia",
  }),
  buildPlayer({
    castaway_id: "US0476",
    full_name: "Tai Trang",
    img: "/images/season_32/Tai-Trang.jpg",
    description:
      "Age: 51 | Hometown: San Francisco, California | Occupation: Gardener",
    age: 51,
    profession: "Gardener",
    hometown: "San Francisco, California",
  }),
  buildPlayer({
    castaway_id: "US0477",
    full_name: "Aubry Bracco",
    img: "/images/season_32/Aubry-Bracco.jpg",
    description:
      "Age: 29 | Hometown: Cambridge, Massachusetts | Occupation: Social Media Marketer",
    age: 29,
    profession: "Social Media Marketer",
    hometown: "Cambridge, Massachusetts",
  }),
  buildPlayer({
    castaway_id: "US0478",
    full_name: "Michele Fitzgerald",
    img: "/images/season_32/Michele-Fitzgerald.jpg",
    description:
      "Age: 24 | Hometown: Freehold, New Jersey | Occupation: Bartender",
    age: 24,
    profession: "Bartender",
    hometown: "Freehold, New Jersey",
  }),
] satisfies Player<CastawayIdType, SeasonNumber>[];

export const SEASON_32_EPISODES = [
  {
    id: "episode_1",
    season_id: "season_32",
    season_num: 32,
    order: 1,
    name: "I'm a Mental Giant",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_2",
    season_id: "season_32",
    season_num: 32,
    order: 2,
    name: "Kindergarten Camp",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_3",
    season_id: "season_32",
    season_num: 32,
    order: 3,
    name: "The Circle of Life",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_4",
    season_id: "season_32",
    season_num: 32,
    order: 4,
    name: "Signed, Sealed and Delivered",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_5",
    season_id: "season_32",
    season_num: 32,
    order: 5,
    name: "The Devils We Know",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_6",
    season_id: "season_32",
    season_num: 32,
    order: 6,
    name: "Play or Go Home",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_7",
    season_id: "season_32",
    season_num: 32,
    order: 7,
    name: "It's Merge Time",
    post_merge: true,
    finale: false,
    merge_occurs: true,
  },
  {
    id: "episode_8",
    season_id: "season_32",
    season_num: 32,
    order: 8,
    name: "The Jocks vs. the Pretty People",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_9",
    season_id: "season_32",
    season_num: 32,
    order: 9,
    name: "It's Psychological Warfare",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_10",
    season_id: "season_32",
    season_num: 32,
    order: 10,
    name: "I'm Not Here to Make Good Friends",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_11",
    season_id: "season_32",
    season_num: 32,
    order: 11,
    name: "It’s a ‘Me’ Game, Not a ‘We’ Game",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_12",
    season_id: "season_32",
    season_num: 32,
    order: 12,
    name: "Now’s the Time to Start Scheming",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_13",
    season_id: "season_32",
    season_num: 32,
    order: 13,
    name: "With Me or Not With Me",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_14",
    season_id: "season_32",
    season_num: 32,
    order: 14,
    name: "Not Going Down Without a Fight",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_15",
    season_id: "season_32",
    season_num: 32,
    order: 15,
    name: "Reunion",
    post_merge: false,
    finale: true,
    merge_occurs: false,
  },
] satisfies Episode<SeasonNumber>[];

export const SEASON_32_CHALLENGES = {
  challenge_0: {
    id: "challenge_0",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 0,
    winning_castaways: [
      "US0463",
      "US0467",
      "US0468",
      "US0470",
      "US0474",
      "US0477",
    ],
  },
  challenge_1: {
    id: "challenge_1",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 1,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_2: {
    id: "challenge_2",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 2,
    winning_castaways: [
      "US0463",
      "US0467",
      "US0468",
      "US0470",
      "US0474",
      "US0477",
    ],
  },
  challenge_3: {
    id: "challenge_3",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 3,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_4: {
    id: "challenge_4",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 4,
    winning_castaways: [
      "US0463",
      "US0467",
      "US0468",
      "US0470",
      "US0474",
      "US0477",
    ],
  },
  challenge_5: {
    id: "challenge_5",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 5,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_6: {
    id: "challenge_6",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 6,
    winning_castaways: [
      "US0463",
      "US0467",
      "US0468",
      "US0470",
      "US0474",
      "US0477",
    ],
  },
  challenge_7: {
    id: "challenge_7",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 7,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_8: {
    id: "challenge_8",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 8,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_9: {
    id: "challenge_9",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 9,
    winning_castaways: ["US0465", "US0471", "US0473", "US0475"],
  },
  challenge_10: {
    id: "challenge_10",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 10,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_11: {
    id: "challenge_11",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 11,
    winning_castaways: ["US0465", "US0471", "US0473", "US0475"],
  },
  challenge_12: {
    id: "challenge_12",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_reward",
    order: 12,
    winning_castaways: ["US0467", "US0468", "US0470", "US0474", "US0477"],
  },
  challenge_13: {
    id: "challenge_13",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_reward",
    order: 13,
    winning_castaways: [
      "US0464",
      "US0466",
      "US0469",
      "US0472",
      "US0476",
      "US0478",
    ],
  },
  challenge_14: {
    id: "challenge_14",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_immunity",
    order: 14,
    winning_castaways: ["US0467", "US0468", "US0470", "US0474", "US0477"],
  },
  challenge_15: {
    id: "challenge_15",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_immunity",
    order: 15,
    winning_castaways: ["US0466", "US0469", "US0472", "US0476", "US0478"],
  },
  challenge_16: {
    id: "challenge_16",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_immunity",
    order: 16,
    winning_castaways: [
      "US0468",
      "US0469",
      "US0470",
      "US0473",
      "US0475",
      "US0478",
    ],
  },
  challenge_17: {
    id: "challenge_17",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_reward",
    order: 17,
    winning_castaways: [
      "US0467",
      "US0471",
      "US0472",
      "US0474",
      "US0476",
      "US0477",
    ],
  },
  challenge_18: {
    id: "challenge_18",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_immunity",
    order: 18,
    winning_castaways: [
      "US0468",
      "US0469",
      "US0470",
      "US0473",
      "US0475",
      "US0478",
    ],
  },
  challenge_19: {
    id: "challenge_19",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    variant: "immunity",
    order: 19,
    winning_castaways: ["US0469"],
  },
  challenge_20: {
    id: "challenge_20",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "team_reward",
    order: 20,
    winning_castaways: ["US0469", "US0470", "US0471", "US0472", "US0476"],
  },
  challenge_21: {
    id: "challenge_21",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "immunity",
    order: 21,
    winning_castaways: ["US0476"],
  },
  challenge_22: {
    id: "challenge_22",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "team_reward",
    order: 22,
    winning_castaways: ["US0471", "US0472", "US0473", "US0476"],
  },
  challenge_23: {
    id: "challenge_23",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "immunity",
    order: 23,
    winning_castaways: ["US0472"],
  },
  challenge_24: {
    id: "challenge_24",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "reward",
    order: 24,
    winning_castaways: ["US0472", "US0476", "US0478"],
  },
  challenge_25: {
    id: "challenge_25",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "immunity",
    order: 25,
    winning_castaways: ["US0473"],
  },
  challenge_26: {
    id: "challenge_26",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "team_reward",
    order: 26,
    winning_castaways: ["US0475", "US0478"],
  },
  challenge_27: {
    id: "challenge_27",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "immunity",
    order: 27,
    winning_castaways: ["US0478"],
  },
  challenge_28: {
    id: "challenge_28",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "team_reward",
    order: 28,
    winning_castaways: ["US0473", "US0476", "US0478"],
  },
  challenge_29: {
    id: "challenge_29",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "immunity",
    order: 29,
    winning_castaways: ["US0475"],
  },
  challenge_30: {
    id: "challenge_30",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "reward",
    order: 30,
    winning_castaways: ["US0474"],
  },
  challenge_31: {
    id: "challenge_31",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "reward",
    order: 31,
    winning_castaways: ["US0477"],
  },
  challenge_32: {
    id: "challenge_32",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "immunity",
    order: 32,
    winning_castaways: ["US0478"],
  },
  challenge_33: {
    id: "challenge_33",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "reward",
    order: 33,
    winning_castaways: ["US0478"],
  },
} satisfies Record<Challenge["id"], Challenge<CastawayIdType, SeasonNumber>>;

export const SEASON_32_ELIMINATIONS = {
  elimination_1: {
    id: "elimination_1",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_1",
    episode_num: 1,
    order: 1,
    castaway_id: "US0461",
    variant: "tribal",
  },
  elimination_2: {
    id: "elimination_2",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_2",
    episode_num: 2,
    order: 2,
    castaway_id: "US0462",
    variant: "tribal",
  },
  elimination_3: {
    id: "elimination_3",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    order: 3,
    castaway_id: "US0463",
    variant: "tribal",
  },
  elimination_4: {
    id: "elimination_4",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_4",
    episode_num: 4,
    order: 4,
    castaway_id: "US0464",
    variant: "medical",
  },
  elimination_5: {
    id: "elimination_5",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_4",
    episode_num: 4,
    order: 5,
    castaway_id: "US0465",
    variant: "tribal",
  },
  elimination_6: {
    id: "elimination_6",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_5",
    episode_num: 5,
    order: 6,
    castaway_id: "US0466",
    variant: "tribal",
  },
  elimination_7: {
    id: "elimination_7",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_6",
    episode_num: 6,
    order: 7,
    castaway_id: "US0467",
    variant: "tribal",
  },
  elimination_8: {
    id: "elimination_8",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    order: 8,
    castaway_id: "US0468",
    variant: "medical",
  },
  elimination_9: {
    id: "elimination_9",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_8",
    episode_num: 8,
    order: 9,
    castaway_id: "US0469",
    variant: "tribal",
  },
  elimination_10: {
    id: "elimination_10",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_9",
    episode_num: 9,
    order: 10,
    castaway_id: "US0470",
    variant: "tribal",
  },
  elimination_11: {
    id: "elimination_11",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_10",
    episode_num: 10,
    order: 11,
    castaway_id: "US0471",
    variant: "tribal",
  },
  elimination_12: {
    id: "elimination_12",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_11",
    episode_num: 11,
    order: 12,
    castaway_id: "US0472",
    variant: "tribal",
  },
  elimination_13: {
    id: "elimination_13",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_12",
    episode_num: 12,
    order: 13,
    castaway_id: "US0473",
    variant: "tribal",
  },
  elimination_14: {
    id: "elimination_14",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_13",
    episode_num: 13,
    order: 14,
    castaway_id: "US0474",
    variant: "medical",
  },
  elimination_15: {
    id: "elimination_15",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    order: 15,
    castaway_id: "US0475",
    variant: "tribal",
  },
  elimination_16: {
    id: "elimination_16",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    order: 16,
    castaway_id: "US0476",
    variant: "final_tribal_council",
  },
  elimination_17: {
    id: "elimination_17",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    order: 17,
    castaway_id: "US0477",
    variant: "final_tribal_council",
  },
} satisfies Record<
  Elimination["id"],
  Elimination<CastawayIdType, SeasonNumber>
>;

export const SEASON_32_EVENTS = {
  event_1: {
    id: "event_1",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0476",
    action: "find_idol",
    multiplier: null,
  },
  event_2: {
    id: "event_2",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_3",
    episode_num: 3,
    castaway_id: "US0473",
    action: "find_idol",
    multiplier: null,
  },
  event_3: {
    id: "event_3",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0471",
    action: "win_idol",
    multiplier: null,
  },
  event_4: {
    id: "event_4",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0471",
    action: "voted_out_with_idol",
    multiplier: null,
  },
  event_5: {
    id: "event_5",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_5",
    episode_num: 5,
    castaway_id: "US0468",
    action: "find_idol",
    multiplier: null,
  },
  event_6: {
    id: "event_6",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_10",
    episode_num: 10,
    castaway_id: "US0476",
    action: "find_extra_vote",
    multiplier: null,
  },
  event_7: {
    id: "event_7",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_12",
    episode_num: 12,
    castaway_id: "US0476",
    action: "use_extra_vote",
    multiplier: null,
  },
  event_8: {
    id: "event_8",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_12",
    episode_num: 12,
    castaway_id: "US0476",
    action: "use_extra_vote",
    multiplier: null,
  },
  event_9: {
    id: "event_9",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0468",
    action: "make_merge",
    multiplier: null,
  },
  event_10: {
    id: "event_10",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0469",
    action: "make_merge",
    multiplier: null,
  },
  event_11: {
    id: "event_11",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0470",
    action: "make_merge",
    multiplier: null,
  },
  event_12: {
    id: "event_12",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0471",
    action: "make_merge",
    multiplier: null,
  },
  event_13: {
    id: "event_13",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0472",
    action: "make_merge",
    multiplier: null,
  },
  event_14: {
    id: "event_14",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0473",
    action: "make_merge",
    multiplier: null,
  },
  event_15: {
    id: "event_15",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0474",
    action: "make_merge",
    multiplier: null,
  },
  event_16: {
    id: "event_16",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0475",
    action: "make_merge",
    multiplier: null,
  },
  event_17: {
    id: "event_17",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0476",
    action: "make_merge",
    multiplier: null,
  },
  event_18: {
    id: "event_18",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0477",
    action: "make_merge",
    multiplier: null,
  },
  event_19: {
    id: "event_19",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_7",
    episode_num: 7,
    castaway_id: "US0478",
    action: "make_merge",
    multiplier: null,
  },
  event_20: {
    id: "event_20",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0478",
    action: "win_survivor",
    multiplier: null,
  },
  event_21: {
    id: "event_21",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0476",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_22: {
    id: "event_22",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0477",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_23: {
    id: "event_23",
    season_id: "season_32",
    season_num: 32,
    episode_id: "episode_14",
    episode_num: 14,
    castaway_id: "US0478",
    action: "make_final_tribal_council",
    multiplier: null,
  },
} satisfies Record<GameEvent["id"], GameEvent<CastawayIdType, SeasonNumber>>;
