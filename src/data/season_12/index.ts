import {
  CastawayLookup,
  Challenge,
  Elimination,
  Episode,
  GameEvent,
  Player,
} from "../../types";

// eslint-disable-next-line @typescript-eslint/no-unused-vars -- used only in typeof for type derivation
const CastawayIds = [
  "US0167",
  "US0168",
  "US0169",
  "US0170",
  "US0171",
  "US0172",
  "US0173",
  "US0174",
  "US0175",
  "US0176",
  "US0177",
  "US0178",
  "US0179",
  "US0180",
  "US0181",
  "US0182",
] as const;

type CastawayIdType = (typeof CastawayIds)[number];

type SeasonNumber = 12;

const buildPlayer = <T extends CastawayIdType>(
  p: { castaway_id: T; full_name: string; img: string } & Partial<
    Omit<
      Player<T, SeasonNumber>,
      "season_id" | "season_num" | "castaway_id" | "full_name" | "img"
    >
  >,
): Player<T, SeasonNumber> => ({
  ...p,
  season_num: 12,
  season_id: "season_12",
});

export const SEASON_12_CASTAWAY_LOOKUP: CastawayLookup = {
  US0167: { full_name: "Tina Scheer", castaway: "Tina" },
  US0168: { full_name: "Melinda Hyder", castaway: "Melinda" },
  US0169: { full_name: "Misty Giles", castaway: "Misty" },
  US0170: { full_name: "Ruth Marie Milliman", castaway: "Ruth Marie" },
  US0171: { full_name: "Bobby Mason", castaway: "Bobby" },
  US0172: { full_name: "Dan Barry", castaway: "Dan" },
  US0173: { full_name: "Nick Stanbury", castaway: "Nick" },
  US0174: { full_name: "Austin Carty", castaway: "Austin" },
  US0175: { full_name: "Sally Schumann", castaway: "Sally" },
  US0176: { full_name: "Bruce Kanegai", castaway: "Bruce" },
  US0177: { full_name: "Courtney Marit", castaway: "Courtney" },
  US0178: { full_name: "Shane Powers", castaway: "Shane" },
  US0179: { full_name: "Cirie Fields", castaway: "Cirie" },
  US0180: { full_name: "Terry Deitz", castaway: "Terry" },
  US0181: { full_name: "Danielle DiLorenzo", castaway: "Danielle" },
  US0182: { full_name: "Aras Baskauskas", castaway: "Aras" },
};

export const SEASON_12_PLAYERS = [
  buildPlayer({
    castaway_id: "US0167",
    full_name: "Tina Scheer",
    img: "/images/season_12/Tina-Scheer.jpg",
    description:
      "Age: 44 | Hometown: Hayward, Wisconsin | Occupation: Lumberjill",
    age: 44,
    profession: "Lumberjill",
    hometown: "Hayward, Wisconsin",
  }),
  buildPlayer({
    castaway_id: "US0168",
    full_name: "Melinda Hyder",
    img: "/images/season_12/Melinda-Hyder.jpg",
    description:
      "Age: 32 | Hometown: Sevierville, Tennessee | Occupation: Singer/Entertainer",
    age: 32,
    profession: "Singer/Entertainer",
    hometown: "Sevierville, Tennessee",
  }),
  buildPlayer({
    castaway_id: "US0169",
    full_name: "Misty Giles",
    img: "/images/season_12/Misty-Giles.jpg",
    description:
      "Age: 24 | Hometown: Dallas, Texas | Occupation: Missile Engineer",
    age: 24,
    profession: "Missile Engineer",
    hometown: "Dallas, Texas",
  }),
  buildPlayer({
    castaway_id: "US0170",
    full_name: "Ruth Marie Milliman",
    img: "/images/season_12/Ruth-Marie-Milliman.jpg",
    description:
      "Age: 48 | Hometown: Greenville, South Carolina | Occupation: Real Estate",
    age: 48,
    profession: "Real Estate",
    hometown: "Greenville, South Carolina",
    nickname: "Ruth Marie",
  }),
  buildPlayer({
    castaway_id: "US0171",
    full_name: "Bobby Mason",
    img: "/images/season_12/Bobby-Mason.jpg",
    description:
      "Age: 32 | Hometown: Los Angeles, California | Occupation: Attorney",
    age: 32,
    profession: "Attorney",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0172",
    full_name: "Dan Barry",
    img: "/images/season_12/Dan-Barry.jpg",
    description:
      "Age: 51 | Hometown: South Hadley, Massachusetts | Occupation: Astronaut",
    age: 51,
    profession: "Astronaut",
    hometown: "South Hadley, Massachusetts",
  }),
  buildPlayer({
    castaway_id: "US0173",
    full_name: "Nick Stanbury",
    img: "/images/season_12/Nick-Stanbury.jpg",
    description:
      "Age: 25 | Hometown: Tempe, Arizona | Occupation: Financial Sales",
    age: 25,
    profession: "Financial Sales",
    hometown: "Tempe, Arizona",
  }),
  buildPlayer({
    castaway_id: "US0174",
    full_name: "Austin Carty",
    img: "/images/season_12/Austin-Carty.jpg",
    description:
      "Age: 24 | Hometown: High Point, North Carolina | Occupation: Writer",
    age: 24,
    profession: "Writer",
    hometown: "High Point, North Carolina",
  }),
  buildPlayer({
    castaway_id: "US0175",
    full_name: "Sally Schumann",
    img: "/images/season_12/Sally-Schumann.jpg",
    description:
      "Age: 27 | Hometown: Chicago, Illinois | Occupation: Social Worker",
    age: 27,
    profession: "Social Worker",
    hometown: "Chicago, Illinois",
  }),
  buildPlayer({
    castaway_id: "US0176",
    full_name: "Bruce Kanegai",
    img: "/images/season_12/Bruce-Kanegai.jpg",
    description:
      "Age: 57 | Hometown: Simi Valley, California | Occupation: Art Teacher",
    age: 57,
    profession: "Art Teacher",
    hometown: "Simi Valley, California",
  }),
  buildPlayer({
    castaway_id: "US0177",
    full_name: "Courtney Marit",
    img: "/images/season_12/Courtney-Marit.jpg",
    description:
      "Age: 31 | Hometown: Los Angeles, California | Occupation: Fire Dancer",
    age: 31,
    profession: "Fire Dancer",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0178",
    full_name: "Shane Powers",
    img: "/images/season_12/Shane-Powers.jpg",
    description:
      "Age: 34 | Hometown: Los Angeles, California | Occupation: Marketing Executive",
    age: 34,
    profession: "Marketing Executive",
    hometown: "Los Angeles, California",
  }),
  buildPlayer({
    castaway_id: "US0179",
    full_name: "Cirie Fields",
    img: "/images/season_12/Cirie-Fields.jpg",
    description:
      "Age: 35 | Hometown: Walterboro, South Carolina | Occupation: Nurse",
    age: 35,
    profession: "Nurse",
    hometown: "Walterboro, South Carolina",
  }),
  buildPlayer({
    castaway_id: "US0180",
    full_name: "Terry Deitz",
    img: "/images/season_12/Terry-Deitz.jpg",
    description:
      "Age: 46 | Hometown: Simsbury, Connecticut | Occupation: Pilot",
    age: 46,
    profession: "Pilot",
    hometown: "Simsbury, Connecticut",
  }),
  buildPlayer({
    castaway_id: "US0181",
    full_name: "Danielle DiLorenzo",
    img: "/images/season_12/Danielle-DiLorenzo.jpg",
    description:
      "Age: 24 | Hometown: Pompano Beach, Florida | Occupation: Medical Sales Rep.",
    age: 24,
    profession: "Medical Sales Rep.",
    hometown: "Pompano Beach, Florida",
  }),
  buildPlayer({
    castaway_id: "US0182",
    full_name: "Aras Baskauskas",
    img: "/images/season_12/Aras-Baskauskas.jpg",
    description:
      "Age: 24 | Hometown: Santa Monica, California | Occupation: Yoga Instructor",
    age: 24,
    profession: "Yoga Instructor",
    hometown: "Santa Monica, California",
  }),
] satisfies Player<CastawayIdType, SeasonNumber>[];

export const SEASON_12_EPISODES = [
  {
    id: "episode_1",
    season_id: "season_12",
    season_num: 12,
    order: 1,
    name: "The First Exile",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_2",
    season_id: "season_12",
    season_num: 12,
    order: 2,
    name: "Breakdown",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_3",
    season_id: "season_12",
    season_num: 12,
    order: 3,
    name: "Crazy Fights, Snake Dinners",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_4",
    season_id: "season_12",
    season_num: 12,
    order: 4,
    name: "Starvation and Lunacy",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_5",
    season_id: "season_12",
    season_num: 12,
    order: 5,
    name: "For Cod's Sake",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_6",
    season_id: "season_12",
    season_num: 12,
    order: 6,
    name: "Salvation and Desertion",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_7",
    season_id: "season_12",
    season_num: 12,
    order: 7,
    name: "A Closer Look",
    post_merge: false,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_8",
    season_id: "season_12",
    season_num: 12,
    order: 8,
    name: "An Emerging Plan",
    post_merge: true,
    finale: false,
    merge_occurs: true,
  },
  {
    id: "episode_9",
    season_id: "season_12",
    season_num: 12,
    order: 9,
    name: "The Power of the Idol",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_10",
    season_id: "season_12",
    season_num: 12,
    order: 10,
    name: "Fight for Your Life or Eat",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_11",
    season_id: "season_12",
    season_num: 12,
    order: 11,
    name: "Medical Emergency",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_12",
    season_id: "season_12",
    season_num: 12,
    order: 12,
    name: "Perilous Scramble",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_13",
    season_id: "season_12",
    season_num: 12,
    order: 13,
    name: "Bamboozled",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_14",
    season_id: "season_12",
    season_num: 12,
    order: 14,
    name: "Call the Whambulence!",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_15",
    season_id: "season_12",
    season_num: 12,
    order: 15,
    name: "The Final Showdown",
    post_merge: true,
    finale: false,
    merge_occurs: false,
  },
  {
    id: "episode_16",
    season_id: "season_12",
    season_num: 12,
    order: 16,
    name: "Reunion",
    post_merge: false,
    finale: true,
    merge_occurs: false,
  },
] satisfies Episode<SeasonNumber>[];

export const SEASON_12_CHALLENGES = {
  challenge_0: {
    id: "challenge_0",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 0,
    winning_castaways: ["US0167", "US0168", "US0170", "US0179"],
  },
  challenge_1: {
    id: "challenge_1",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 1,
    winning_castaways: ["US0172", "US0176", "US0178", "US0180"],
  },
  challenge_2: {
    id: "challenge_2",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_reward",
    order: 2,
    winning_castaways: ["US0171", "US0173", "US0174", "US0182"],
  },
  challenge_3: {
    id: "challenge_3",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 3,
    winning_castaways: ["US0169", "US0175", "US0177", "US0181"],
  },
  challenge_4: {
    id: "challenge_4",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 4,
    winning_castaways: ["US0172", "US0176", "US0178", "US0180"],
  },
  challenge_5: {
    id: "challenge_5",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    variant: "team_immunity",
    order: 5,
    winning_castaways: ["US0171", "US0173", "US0174", "US0182"],
  },
  challenge_6: {
    id: "challenge_6",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_reward",
    order: 6,
    winning_castaways: [
      "US0169",
      "US0170",
      "US0172",
      "US0173",
      "US0174",
      "US0175",
      "US0180",
    ],
  },
  challenge_7: {
    id: "challenge_7",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_2",
    episode_num: 2,
    variant: "team_immunity",
    order: 7,
    winning_castaways: [
      "US0169",
      "US0170",
      "US0172",
      "US0173",
      "US0174",
      "US0175",
      "US0180",
    ],
  },
  challenge_8: {
    id: "challenge_8",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_reward",
    order: 8,
    winning_castaways: [
      "US0169",
      "US0170",
      "US0172",
      "US0173",
      "US0174",
      "US0175",
      "US0180",
    ],
  },
  challenge_9: {
    id: "challenge_9",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_3",
    episode_num: 3,
    variant: "team_immunity",
    order: 9,
    winning_castaways: [
      "US0171",
      "US0176",
      "US0177",
      "US0178",
      "US0179",
      "US0181",
      "US0182",
    ],
  },
  challenge_10: {
    id: "challenge_10",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_reward",
    order: 10,
    winning_castaways: [
      "US0171",
      "US0176",
      "US0177",
      "US0178",
      "US0179",
      "US0181",
      "US0182",
    ],
  },
  challenge_11: {
    id: "challenge_11",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_4",
    episode_num: 4,
    variant: "team_immunity",
    order: 11,
    winning_castaways: [
      "US0171",
      "US0176",
      "US0177",
      "US0178",
      "US0179",
      "US0181",
      "US0182",
    ],
  },
  challenge_12: {
    id: "challenge_12",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_reward",
    order: 12,
    winning_castaways: [
      "US0171",
      "US0176",
      "US0177",
      "US0178",
      "US0179",
      "US0181",
      "US0182",
    ],
  },
  challenge_13: {
    id: "challenge_13",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_5",
    episode_num: 5,
    variant: "team_immunity",
    order: 13,
    winning_castaways: ["US0172", "US0173", "US0174", "US0175", "US0180"],
  },
  challenge_14: {
    id: "challenge_14",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_immunity",
    order: 14,
    winning_castaways: [
      "US0176",
      "US0177",
      "US0178",
      "US0179",
      "US0181",
      "US0182",
    ],
  },
  challenge_15: {
    id: "challenge_15",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_6",
    episode_num: 6,
    variant: "team_reward",
    order: 15,
    winning_castaways: [
      "US0176",
      "US0177",
      "US0178",
      "US0179",
      "US0181",
      "US0182",
    ],
  },
  challenge_16: {
    id: "challenge_16",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    variant: "immunity",
    order: 16,
    winning_castaways: ["US0180"],
  },
  challenge_17: {
    id: "challenge_17",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "team_reward",
    order: 17,
    winning_castaways: ["US0175", "US0176", "US0182"],
  },
  challenge_18: {
    id: "challenge_18",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_9",
    episode_num: 9,
    variant: "immunity",
    order: 18,
    winning_castaways: ["US0180"],
  },
  challenge_19: {
    id: "challenge_19",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "team_reward",
    order: 19,
    winning_castaways: ["US0175", "US0176", "US0177", "US0180"],
  },
  challenge_20: {
    id: "challenge_20",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_10",
    episode_num: 10,
    variant: "immunity",
    order: 20,
    winning_castaways: ["US0180"],
  },
  challenge_21: {
    id: "challenge_21",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_11",
    episode_num: 11,
    variant: "reward",
    order: 21,
    winning_castaways: ["US0179"],
  },
  challenge_22: {
    id: "challenge_22",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "team_reward",
    order: 22,
    winning_castaways: ["US0177", "US0180", "US0181"],
  },
  challenge_23: {
    id: "challenge_23",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_12",
    episode_num: 12,
    variant: "immunity",
    order: 23,
    winning_castaways: ["US0180"],
  },
  challenge_24: {
    id: "challenge_24",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "reward",
    order: 24,
    winning_castaways: ["US0180"],
  },
  challenge_25: {
    id: "challenge_25",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_13",
    episode_num: 13,
    variant: "immunity",
    order: 25,
    winning_castaways: ["US0180"],
  },
  challenge_26: {
    id: "challenge_26",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "reward",
    order: 26,
    winning_castaways: ["US0182"],
  },
  challenge_27: {
    id: "challenge_27",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_14",
    episode_num: 14,
    variant: "immunity",
    order: 27,
    winning_castaways: ["US0182"],
  },
  challenge_28: {
    id: "challenge_28",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    variant: "reward",
    order: 28,
    winning_castaways: ["US0180"],
  },
  challenge_29: {
    id: "challenge_29",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    variant: "immunity",
    order: 29,
    winning_castaways: ["US0181"],
  },
} satisfies Record<Challenge["id"], Challenge<CastawayIdType, SeasonNumber>>;

export const SEASON_12_ELIMINATIONS = {
  elimination_1: {
    id: "elimination_1",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_1",
    episode_num: 1,
    order: 1,
    castaway_id: "US0167",
    variant: "tribal",
  },
  elimination_2: {
    id: "elimination_2",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_2",
    episode_num: 2,
    order: 2,
    castaway_id: "US0168",
    variant: "tribal",
  },
  elimination_3: {
    id: "elimination_3",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_3",
    episode_num: 3,
    order: 3,
    castaway_id: "US0169",
    variant: "tribal",
  },
  elimination_4: {
    id: "elimination_4",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_4",
    episode_num: 4,
    order: 4,
    castaway_id: "US0170",
    variant: "tribal",
  },
  elimination_5: {
    id: "elimination_5",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_5",
    episode_num: 5,
    order: 5,
    castaway_id: "US0171",
    variant: "tribal",
  },
  elimination_6: {
    id: "elimination_6",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_6",
    episode_num: 6,
    order: 6,
    castaway_id: "US0172",
    variant: "tribal",
  },
  elimination_7: {
    id: "elimination_7",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    order: 7,
    castaway_id: "US0173",
    variant: "tribal",
  },
  elimination_8: {
    id: "elimination_8",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_9",
    episode_num: 9,
    order: 8,
    castaway_id: "US0174",
    variant: "tribal",
  },
  elimination_9: {
    id: "elimination_9",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_10",
    episode_num: 10,
    order: 9,
    castaway_id: "US0175",
    variant: "tribal",
  },
  elimination_10: {
    id: "elimination_10",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_11",
    episode_num: 11,
    order: 10,
    castaway_id: "US0176",
    variant: "medical",
  },
  elimination_11: {
    id: "elimination_11",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_12",
    episode_num: 12,
    order: 11,
    castaway_id: "US0177",
    variant: "tribal",
  },
  elimination_12: {
    id: "elimination_12",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_13",
    episode_num: 13,
    order: 12,
    castaway_id: "US0178",
    variant: "tribal",
  },
  elimination_13: {
    id: "elimination_13",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_14",
    episode_num: 14,
    order: 13,
    castaway_id: "US0179",
    variant: "tribal",
  },
  elimination_14: {
    id: "elimination_14",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    order: 14,
    castaway_id: "US0180",
    variant: "tribal",
  },
  elimination_15: {
    id: "elimination_15",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    order: 15,
    castaway_id: "US0181",
    variant: "final_tribal_council",
  },
} satisfies Record<
  Elimination["id"],
  Elimination<CastawayIdType, SeasonNumber>
>;

export const SEASON_12_EVENTS = {
  event_1: {
    id: "event_1",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_4",
    episode_num: 4,
    castaway_id: "US0180",
    action: "find_idol",
    multiplier: null,
  },
  event_2: {
    id: "event_2",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0173",
    action: "make_merge",
    multiplier: null,
  },
  event_3: {
    id: "event_3",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0174",
    action: "make_merge",
    multiplier: null,
  },
  event_4: {
    id: "event_4",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0175",
    action: "make_merge",
    multiplier: null,
  },
  event_5: {
    id: "event_5",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0176",
    action: "make_merge",
    multiplier: null,
  },
  event_6: {
    id: "event_6",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0177",
    action: "make_merge",
    multiplier: null,
  },
  event_7: {
    id: "event_7",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0178",
    action: "make_merge",
    multiplier: null,
  },
  event_8: {
    id: "event_8",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0179",
    action: "make_merge",
    multiplier: null,
  },
  event_9: {
    id: "event_9",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0180",
    action: "make_merge",
    multiplier: null,
  },
  event_10: {
    id: "event_10",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0181",
    action: "make_merge",
    multiplier: null,
  },
  event_11: {
    id: "event_11",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_8",
    episode_num: 8,
    castaway_id: "US0182",
    action: "make_merge",
    multiplier: null,
  },
  event_12: {
    id: "event_12",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0182",
    action: "win_survivor",
    multiplier: null,
  },
  event_13: {
    id: "event_13",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0181",
    action: "make_final_tribal_council",
    multiplier: null,
  },
  event_14: {
    id: "event_14",
    season_id: "season_12",
    season_num: 12,
    episode_id: "episode_15",
    episode_num: 15,
    castaway_id: "US0182",
    action: "make_final_tribal_council",
    multiplier: null,
  },
} satisfies Record<GameEvent["id"], GameEvent<CastawayIdType, SeasonNumber>>;
